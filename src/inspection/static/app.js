const $=s=>document.querySelector(s);let recordings=[],scenarios=[],page=1,operatorToken='',lidarState=null,acceptanceState=null,preflightState=null,profileState=null,workbenchState=null;
...
const initialTab=SandboxUIState.tabFromHash(location.hash);activateTab(initialTab,false);
...
function updateOperatorInputs(){const action=$('#operator-action').value,smoke=action==='flight-smoke',simulator=smoke||action==='collection-single'||action==='collection-gate';$('#operator-scenario').disabled=!smoke;$('#operator-scenario').hidden=!smoke;$('#operator-display-mode').disabled=!simulator;$('#operator-display-mode').hidden=!simulator}
...
$('#operator-start').onclick=()=>{const action=$('#operator-action').value,scenario=action==='flight-smoke'?$('#operator-scenario').value:null,simulator=action==='flight-smoke'||action==='collection-single'||action==='collection-gate',parameters=simulator?{display_mode:$('#operator-display-mode').value}:null;startManaged(action,scenario,parameters)};
...
$('#workbench-start').onclick=async()=>{const parameters={experiment_id:$('#workbench-name').value.trim(),dataset_id:$('#workbench-dataset').value,preset:$('#workbench-preset').value,epochs:Number($('#workbench-epochs').value),imgsz:Number($('#workbench-imgsz').value),batch:Number($('#workbench-batch').value),device:$('#workbench-device').value};const accepted=await requestConfirmation({title:'Start visual training?',detail:'This creates a development recipe, trains YOLO11n, exports ONNX, and replays the fixed validation membership.',facts:{Experiment:parameters.experiment_id,Dataset:parameters.dataset_id,Preset:parameters.preset,Epochs:parameters.epochs},confirmLabel:'Create and train'});if(!accepted)return;try{await post('/api/operator/start',{action:'workbench-run',parameters});$('#workbench-name').value='';await refreshOperator()}catch(e){actionError(e.message)}};
