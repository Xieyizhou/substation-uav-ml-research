...
const workbenchPresets={smoke:{epochs:1,imgsz:320,batch:4},quick:{epochs:10,imgsz:416,batch:8},full:{epochs:100,imgsz:640,batch:8}};
function workbenchRunName(){const d=new Date(),part=n=>String(n).padStart(2,'0');return `visual-${$('#workbench-preset').value}-${d.getFullYear()}${part(d.getMonth()+1)}${part(d.getDate())}-${part(d.getHours())}${part(d.getMinutes())}`}
function applyWorkbenchPreset(){const value=workbenchPresets[$('#workbench-preset').value];$('#workbench-epochs').value=value.epochs;$('#workbench-imgsz').value=value.imgsz;$('#workbench-batch').value=value.batch;if(!$('#workbench-name').value)$('#workbench-name').value=workbenchRunName()}
...
function renderWorkbench(value){
  ...
  const parentSelect=$('#workbench-parent'),selectedParent=parentSelect.value;
  parentSelect.innerHTML='<option value="">YOLO11n pretrained</option>'+(value.verified_models||[]).map(row=>`<option value="${esc(row.experiment_id)}">Fine-tune: ${esc(row.experiment_id)}</option>`).join('');
  if((value.verified_models||[]).some(row=>row.experiment_id===selectedParent))parentSelect.value=selectedParent;
  ...
  document.querySelectorAll('.dataset-row').forEach(button=>button.onclick=()=>{selectedWorkbenchDatasetId=button.dataset.dataset;$('#workbench-dataset').value=selectedWorkbenchDatasetId;renderWorkbench(workbenchState)});
  ...
  const rows=value.runs||[],latest=rows.filter(row=>row.state==='running').at(-1)||rows.at(-1),filter=$('#workbench-run-filter').value.trim().toLowerCase();
  if(latest){const progress=Math.max(0,Math.min(1,Number(latest.progress||0)));$('#workbench-active-copy').textContent=`${latest.experiment_id} · ${latest.stage} · ${latest.epoch||0}/${latest.total_epochs||'—'} epochs · ${latest.state==='complete'?'Complete':eta(latest.eta_seconds)}`;$('#workbench-progress').innerHTML=`<div class="progress-track"><div class="progress-fill" style="width:${progress*100}%"></div></div>${latest.error?`<p class="experiment-error">${esc(latest.failure_code)} · ${esc(latest.error)}</p>`:''}`;$('#workbench-resume').hidden=!(latest.state==='failed'&&latest.checkpoint_available);$('#workbench-resume').dataset.experiment=latest.experiment_id}else{$('#workbench-progress').innerHTML='';$('#workbench-resume').hidden=true}
  const filtered=rows.slice().reverse().filter(row=>!filter||`${row.experiment_id} ${row.dataset_id}`.toLowerCase().includes(filter)),visible=showAllRuns?filtered:filtered.slice(0,6);
  $('#workbench-runs').innerHTML=visible.length?visible.map(row=>{const m=row.metrics||{},t=row.timing||{},delta=row.comparison||{};return `<article class="experiment-card"><header><span class="badge ${row.state==='complete'?'pass':row.state==='failed'?'failure':'warning'}">${esc(row.state)}</span><div><h3>${esc(row.experiment_id)}</h3><small>${esc(row.dataset_id)} · ${esc(row.preset)} · ${esc(row.parameters.imgsz)} px</small></div></header><div class="experiment-facts"><span>mAP50–95<b>${pct(m.map50_95)}</b></span><span>Macro F1<b>${pct(m.macro_f1)}</b></span><span>Recall<b>${pct(m.recall)}</b></span><span>Small recall<b>${pct(m.small_object_recall)}</b></span><span>No-target FPR<b>${pct(m.no_target_false_positive_rate)}</b></span><span>Replay P95<b>${fixed(t.p95_ms)} ms</b></span></div>${row.comparison_status==='complete'?`<p class="comparison-note">Compared with ${esc(row.comparison_source)} · Δ macro-F1 ${pct(delta.macro_f1)} · small recall ${pct(delta.small_object_recall)} · no-target FPR ${pct(delta.no_target_false_positive_rate)}<br><small>${esc(row.comparison_scope||'Same development validation view')}</small></p>`:''}${row.error?`<p class="experiment-error">${esc(row.error)}</p>`:''}</article>`}).join(''):'<article class="experiment-empty">No matching model workbench run.</article>';
...
function renderWorkbenchDataset(row){
  if(!row){$('#dataset-audit').textContent='No dataset selected.';$('#dataset-audit-status').textContent='Not checked';return;}
  const coverage=['transformer','switchgear','capacitor_bank','reactor'].every(name=>(row.class_counts?.validation?.[name]||0)>0);
  $('#dataset-audit').innerHTML=`<p>Registration status: <b>${esc(row.status)}</b></p><p>Four-class validation coverage: <b>${coverage?'Present':'Incomplete'}</b></p><p>${row.source_type==='reviewed_feedback'?'Reviewed feedback uses fixed base validation. Image, label and provenance hashes are checked again before training.':'Registration metadata is available. This page does not recheck every image or prove source-group isolation.'}</p>${(row.issues||[]).map(issue=>`<p>${esc(issue)}</p>`).join('')}`;
  $('#dataset-audit-status').textContent='Registration metadata';
  ...
  $('#dataset-registration-status').textContent=row.status||'Unknown';
  $('#dataset-registration-status').className='badge '+(row.status==='ready'?'pass':'warning');
  ...
  $('#dataset-source-type').textContent=({native_training_view:'Native training view',reviewed_feedback:'Reviewed feedback',imported_yolo:'Imported YOLO view'})[row.source_type]||row.source_type;
...
function renderWorkbenchInference(value){
  ...
  const options=models.map(model=>`<option value="${esc(model.experiment_id)}">${esc(model.experiment_id)} · ${esc(model.imgsz)} px · threshold ${fixed(model.threshold)}</option>`).join('');
  $('#inference-primary').innerHTML=options;
  $('#inference-comparison').innerHTML='<option value="">No comparison</option>'+options;
  if(models.some(model=>model.experiment_id===selected))$('#inference-primary').value=selected;
  if(models.some(model=>model.experiment_id===comparison))$('#inference-comparison').value=comparison;
  $('#workbench-infer').disabled=!models.length;
  const rows=(value.inferences||[]).slice().reverse(),visible=showAllInferences?rows:rows.slice(0,3);
  $('#workbench-inferences').innerHTML=visible.length?visible.map(row=>{const entries=Object.entries(row.results||{});return `<section class="inference-result"><div class="section-head"><div><h3>${esc(row.inference_id)}</h3><p>${esc(row.input_width)} × ${esc(row.input_height)} · ${esc(row.created_at)}</p></div></div><div class="inference-images">${entries.map(([role,result])=>`<figure><img loading="lazy" src="/api/workbench/inference/${encodeURIComponent(row.inference_id)}/${esc(result.annotated_image)}" alt="${esc(role)} model inference"><figcaption><b>${esc(role)} · ${esc(result.experiment_id)}</b> · ${esc(result.detection_count)} detections · ${fixed(result.latency_ms)} ms load + first inference · threshold ${fixed(result.threshold)}</figcaption></figure>`).join('')}</div></section>`}).join(''):'<p class="experiment-empty">No local image inference has run.</p>';
  $('#workbench-inferences-show-all').hidden=rows.length<=3;$('#workbench-inferences-show-all').textContent=showAllInferences?'Show recent':'Show all';
}
...
const bytes=value=>{const number=Number(value||0);if(number>=1024**3)return `${(number/1024**3).toFixed(2)} GiB`;if(number>=1024**2)return `${(number/1024**2).toFixed(1)} MiB`;return `${Math.round(number/1024)} KiB`};
function renderStorage(value){
  const percent=value.disk_total_bytes?100*value.disk_free_bytes/value.disk_total_bytes:0;
  $('#home-storage-title').textContent=`${bytes(value.disk_free_bytes)} free`;
...
function renderOperator(value){
  ...
  globalThis.renderLiveReplanOperator?.(active);
  globalThis.renderSemanticOperator?.(active);
  globalThis.renderEvidenceOperator?.(active);
  globalThis.renderFeedbackOperator?.(active);
  ...
  $('#workbench-resume').disabled=Boolean(active);
  $('#workbench-infer').disabled=Boolean(active)||!(workbenchState?.verified_models||[]).length;
...
$('#fly-gate-start').onclick=()=>startManaged('collection-gate',null,{display_mode:$('#operator-display-mode').value});
...
$('#workbench-preset').onchange=()=>{$('#workbench-name').value='';applyWorkbenchPreset()};
$('#workbench-import').onclick=()=>{const query=new URLSearchParams();['transformer','switchgear','capacitor_bank','reactor'].forEach(name=>query.set(name,$(`#map-${name}`).value));window.location.href=`uav-sandbox://import-yolo?${query}`};
$('#workbench-start').onclick=async()=>{const parameters={experiment_id:$('#workbench-name').value.trim(),dataset_id:$('#workbench-dataset').value,preset:$('#workbench-preset').value,epochs:Number($('#workbench-epochs').value),imgsz:Number($('#workbench-imgsz').value),batch:Number($('#workbench-batch').value),device:$('#workbench-device').value};if($('#workbench-parent').value)parameters.parent_experiment_id=$('#workbench-parent').value;if(Number($('#workbench-freeze').value)>0)parameters.freeze=Number($('#workbench-freeze').value);const accepted=await requestConfirmation({title:'Start visual training?',detail:'This creates a development recipe, trains YOLO11n, exports ONNX, and replays the fixed validation membership.',facts:{Experiment:parameters.experiment_id,Dataset:parameters.dataset_id,Preset:parameters.preset,Epochs:parameters.epochs,'Start from':parameters.parent_experiment_id||'YOLO11n pretrained','训练范围':parameters.freeze?'冻结前 '+parameters.freeze+' 层':'全部层'},confirmLabel:'Create and train'});if(!accepted)return;try{await post('/api/operator/start',{action:'workbench-run',parameters});$('#workbench-name').value='';await refreshOperator()}catch(e){actionError(e.message)}};
$('#workbench-resume').onclick=async()=>{const experiment_id=$('#workbench-resume').dataset.experiment;if(!experiment_id)return;const accepted=await requestConfirmation({title:'Resume training checkpoint?',detail:'The existing last.pt checkpoint and frozen recipe will be used. Parameters cannot change during resume.',facts:{Experiment:experiment_id},confirmLabel:'Resume'});if(!accepted)return;try{await post('/api/operator/start',{action:'workbench-resume',parameters:{experiment_id}});await refreshOperator()}catch(e){actionError(e.message)}};
$('#workbench-infer').onclick=()=>{const primary=$('#inference-primary').value,comparison=$('#inference-comparison').value;if(!primary)return;if(comparison===primary){actionError('Choose a different comparison model.');return}const query=new URLSearchParams({experiment_id:primary});if(comparison)query.set('comparison_experiment_id',comparison);window.location.href=`uav-sandbox://infer-image?${query}`};
