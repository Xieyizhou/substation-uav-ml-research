...
function renderAcceptance(value){
  acceptanceState=value;const gate=value.acceptance,labels={visual_replay:'Visual replay',lidar_replay:'LiDAR replay',closed_loop_flight:'Closed-loop flight',safe_process_stop:'Safe process stop',failure_diagnostics:'Failure diagnostics'};
...
async function refreshOperator(){try{const [value,experiments,lidar,acceptance]=await Promise.all([api('/api/operator'),api('/api/experiments'),api('/api/lidar'),api('/api/acceptance')]);operatorToken=value.operator_token;renderExperiments(experiments);renderLidar(lidar);renderAcceptance(acceptance);renderOperator(value)}catch(e){$('#operator-state').textContent=e.message}}
