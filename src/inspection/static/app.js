...
async function refresh(){
  const [profile,dash,runtime,doctor,recs,available,operator,research,experiments,lidar,acceptance,preflight]=await Promise.all([api('/api/profile'),api('/api/dashboard'),api('/api/runtime'),api('/api/doctor'),api('/api/recordings'),api('/api/scenarios'),api('/api/operator'),api('/api/research'),api('/api/experiments'),api('/api/lidar'),api('/api/acceptance'),api('/api/preflight')]);
  recordings=recs;scenarios=available;operatorToken=operator.operator_token;renderProfile(profile);renderDashboard(dash,runtime);renderResearch(research);renderExperiments(experiments);renderLidar(lidar);renderAcceptance(acceptance);renderPreflight(preflight);renderDoctor(doctor);renderSelectors();renderOperator(operator);
