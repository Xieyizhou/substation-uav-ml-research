...
def replay(out):
    ...
    inputs=[journal,out/'protocol.json',Path(__file__).resolve(),Path(__file__).with_name('envelope_route.py'),Path(__file__).with_name('live_obstacle_map.py')]+snapshots
