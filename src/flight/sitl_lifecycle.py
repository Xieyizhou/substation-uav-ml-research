...
import shutil
import time
from src.flight.sitl_support import preflight, stop, command, guard, check_spawn, collision_boxes, inside
...
async def run(root, px4, assets, out, fly=False, preflight_hook=None, takeoff_altitude=1., post_hover_hook=None, server_config=None, post_hover_timeout_s=5., source_world=None, visual_service=None):
    ROOT, PX4, OUT = Path(root), Path(px4), Path(out)
    ...
    binary=preflight(BUILD);config_path=assets/'obstacles.json';check_spawn(json.loads(config_path.read_text()))
    ...
    try:
        ...
        while '/research_camera/image' not in await command('gz','topic','-l'):
            ...
            if processes[0].returncode is not None:raise RuntimeError('Gazebo exited')
