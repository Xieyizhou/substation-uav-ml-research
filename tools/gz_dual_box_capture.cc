...
template<class T> double stamp(const T &m) {return m.header().stamp().sec()+m.header().stamp().nsec()*1e-9;}
template<class T> void json(const T &m,const std::string &path) {
...
int main(int argc,char **argv) {
  ...
  std::deque<gz::msgs::AnnotatedAxisAligned2DBox_V> full,visible;
  ...
  std::function<void(const gz::msgs::AnnotatedAxisAligned2DBox_V&)> fc=[&](const auto&m){receive(full,m);},vc=[&](const auto&m){receive(visible,m);};
  gz::transport::Node node;
  if(!node.Subscribe("/research_camera/image",rc)||!node.Subscribe("/research_camera/depth",dc)||!node.Subscribe("/diagnostic/instances/labels_map",mc)||!node.Subscribe("/research_camera/boxes",fc)||!node.Subscribe("/diagnostic/visible_boxes",vc)||!node.Subscribe("/world/"+world+"/pose/info",pc))return 3;
  ...
  while(std::chrono::steady_clock::now()-start<std::chrono::seconds(60)&&count<12){
    ...
    for(const auto&r:rgb){
      ...
      auto d=nearest(depth);auto m=nearest(mask);auto p=nearest(poses);auto b=nearest(full);auto v=nearest(visible);
      if(d==depth.end()||m==mask.end()||p==poses.end()||b==full.end()||v==visible.end())continue;
      ...
      for(auto pair:{std::make_pair(&r,"rgb"),std::make_pair(&*d,"depth"),std::make_pair(&*m,"mask")}){
        auto metadata=*pair.first;auto data=metadata.data();metadata.clear_data();json(metadata,prefix+"-"+pair.second+".json");
        std::ofstream f(prefix+"-"+pair.second+".bin",std::ios::binary);f.write(data.data(),data.size());
      }
      json(*p,prefix+"-pose.json");json(*b,prefix+"-boxes.json");json(*v,prefix+"-visible-boxes.json");
