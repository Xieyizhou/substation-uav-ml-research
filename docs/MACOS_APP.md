...
```bash
./scripts/package_macos_release.sh 0.6.2
cd dist/releases
shasum -a 256 -c UAV-Research-Sandbox-v0.6.2-macos-*-SHA256SUMS
```
...
```bash
./scripts/package_macos_beta.sh 0.6.2
cd dist/releases
shasum -a 256 -c UAV-Research-Sandbox-v0.6.2-macos-*-SHA256SUMS
```
...
MACOS_NOTARY_PROFILE="uav-sandbox-notary" \
  ./scripts/package_macos_notarized_beta.sh 0.6.2
```
