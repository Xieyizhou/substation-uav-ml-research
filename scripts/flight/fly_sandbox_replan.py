...
from scripts.flight import sandbox_replan_lifecycle as trial
...
GOAL=(1.7,6.9)
INITIAL_ROUTE=[(1.5,1.5),(1.6,.9),(1.6,6.9),GOAL]
CASE='baseline'
...
async def main():
        ...
        try:
            ...
            while index<len(route)-1:
                ...
                if stage=='following' and mapping.route_blocked(remaining):
                    ...
                    event('obstacle_detected_stop_requested',speed_m_s=speed,lidar_m=distance,observed_returns=len(mapping.points),map_snapshot=snapshot)
    ...
    try:await trial.main(fly=True,preflight_hook=gate,takeoff_altitude=2.,post_hover_hook=follow,server_config=OUT.parent/'magnetic-contract-001/server.config',post_hover_timeout_s=180.,source_world=OUT.parent/'envelope-aware-scene-v1/world.sdf')
...
if __name__=='__main__':
    ...
    if not args.fly:
        ...
        subprocess.run([sys.executable,'-m','scripts.flight.audit_lowload_replan',str(OUT)],check=True,cwd=trial.ROOT)
        if json.loads((OUT/'completion.json').read_text())['status']!='sensor_stop_replan_resume_goal_verified':raise RuntimeError('Flight audit failed')
