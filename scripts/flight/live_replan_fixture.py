"""Test harness only. Fixture coordinates must not enter the flight planner."""
import asyncio
import json
from src.ml.artifacts import file_sha256
from src.vision.canonical.plan import write_record

async def insert(out):
    sdf='<sdf version="1.9"><model name="unregistered_pillar"><static>true</static><pose>-8.85 -6.3 2 0 0 0</pose><link name="pillar"><collision name="collision"><geometry><box><size>0.3 0.3 4</size></box></geometry></collision><visual name="visual"><geometry><box><size>0.3 0.3 4</size></box></geometry></visual></link></model></sdf>'
    ...
    if p.returncode or 'data: true' not in stdout.decode():raise RuntimeError('Fixture creation failed')
