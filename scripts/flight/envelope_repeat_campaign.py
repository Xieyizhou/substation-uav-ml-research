...
def freeze():
    ...
    units=[dict(case='no_path',repeat=1,directory='envelope-repeat-v1-no_path-1',expected='safe_rejection')]
    units += [dict(case=c,repeat=n,directory=f'envelope-repeat-v1-{c}-{n}',expected='goal') for c in ('baseline','west','later') for n in (1,2)]
    sources=[Path(__file__).resolve()]+[ROOT/'scripts/flight'/name for name in ('fly_envelope_replan.py','envelope_fixture.py','audit_envelope_replan.py','envelope_trial_lifecycle.py','fly_pillar_route.py','check_final_heading.py','retest_sim_heading.py','prepare_avoidance_route.py','check_px4_shadow.py','measure_mapping_error.py')]
    ...
    write_record(OUT/'protocol.json',dict(version='envelope-repeat-v1',resource_preflight_wait_s=60,change='explicit 1.8+.18+.20m envelope; separate layout, original narrow layout retains rejection; complete scan replay and per-flight empirical map budget audit',cases=CASES,units=units,harness_feasibility=feasibility,positive_required=6,negative_required=1,max_attempts_per_unit=1,stop_on_any_unexpected_result=True,pass_rule='all six complete sensor stop/replan/resume/goal and audited landing; negative trial reproduces no path or blocked goal and safely lands; all hashes valid',scope='PX4 SITL only; stationary newly inserted pillar; no physical flight',sandbox_enable_requires_all=True,training_admitted=False,promotable=False,inputs={str(p):file_sha256(p) for p in sources}))
            ...
            if not out.exists():
                ...
                while True:
                    try:preflight();break
                        ...
                        time.sleep(1)
                command=[sys.executable,'-m','scripts.flight.fly_envelope_replan','--case',unit['case'],'--repeat',str(unit['repeat'])]
                subprocess.run(command,check=True,cwd=ROOT)
                subprocess.run(command+['--fly'],check=True,cwd=ROOT)
            if not completion.exists():
                replay(out)
                if unit['expected']=='goal':
                    subprocess.run([sys.executable,'-m','scripts.flight.audit_envelope_replan',str(out)],check=True,cwd=ROOT)
