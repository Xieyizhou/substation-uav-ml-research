...
from scripts.flight import startup_trial_lifecycle as trial
...
async def main():
    ...
    campaign=json.loads((OUT.parent/'startup-repeat-v2/protocol.json').read_text())
...
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--fly',action='store_true');p.add_argument('--case',choices=tuple(fixture_harness.CASES),required=True);p.add_argument('--repeat',type=int,choices=(1,2),required=True);args=p.parse_args();CASE=args.case;OUT=OUT.parent/f'startup-repeat-v2-{CASE}-{args.repeat}'
