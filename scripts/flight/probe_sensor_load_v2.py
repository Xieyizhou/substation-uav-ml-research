...
def prepare(profile):
    ...
    paths=[source,src,world,model/'model.sdf',model/'model.config',Path(__file__).resolve(),ROOT/'src/sensors/gazebo_lidar_observed.py',ROOT/'src/sensors/gazebo_lidar.py']
...
async def run(profile):
    ...
    try:
        ...
        source._received_times.reset_measurement()
        ...
        write_record(out/'samples.json',dict(rows=rows,poses=events,subscriber_diagnostics=diagnostics))
        write_record(out/'completion.json',dict(status='probe_complete' if error is None else 'failed',error=error,px4_started=False,flight_started=False,subscriber_diagnostics=diagnostics,max_observer_poll_gap_s=max(gaps,default=None),unhealthy_samples=sum(not r['healthy'] for r in rows),owned_processes_exited=all(p.returncode is not None for p in processes),limitation='static transport/load experiment; not a successful flight or universal timing guarantee',training_admitted=False,promotable=False,inputs={str(p):file_sha256(p) for p in [out/'protocol.json',out/'samples.json']}))
