...
def freeze():
    ...
    units=[dict(case='no_path',repeat=1,directory='replan-capture-v3-no_path-1',expected='safe_rejection')]
    units += [dict(case=c,repeat=n,directory=f'replan-capture-v3-{c}-{n}',expected='goal') for c in ('baseline','west','later') for n in (1,2)]
    sources=[Path(__file__).resolve()]+[ROOT/'scripts/flight'/name for name in ('fly_replan_capture.py','replan_repeat_fixture.py','audit_replan_capture.py','route_trial_lifecycle.py','fly_pillar_route.py','check_final_heading.py','retest_sim_heading.py','prepare_avoidance_route.py')]
    ...
    write_record(OUT/'protocol.json',dict(version='replan-capture-v3',change='v2 clearance preference and full evidence; bounded position capture within 0.08m of waypoint, fail if drift exceeds 0.12m; unchanged 0.03m arrival acceptance, 1.8m exclusion and 3 replans',cases=CASES,units=units,harness_feasibility=feasibility,positive_required=6,negative_required=1,max_attempts_per_unit=1,stop_on_any_unexpected_result=True,pass_rule='all six complete sensor stop/replan/resume/goal and audited landing; no-path trial reproduces no path and safely lands; all hashes valid',scope='PX4 SITL only; stationary newly inserted pillar; no physical flight',sandbox_enable_requires_all=True,training_admitted=False,promotable=False,inputs={str(p):file_sha256(p) for p in sources}))
...
def run():
    ...
    for unit in protocol['units']:
        ...
        try:
            if not out.exists():
                command=[sys.executable,'-m','scripts.flight.fly_replan_capture','--case',unit['case'],'--repeat',str(unit['repeat'])]
