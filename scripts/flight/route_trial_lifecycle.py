"""Independent bounded SITL route lifecycle; vision never controls flight."""
import asyncio
import argparse
import json
import math
...
async def main(fly=False, preflight_hook=None, takeoff_altitude=1., post_hover_hook=None, server_config=None, post_hover_timeout_s=5.):
    ...
    try:
        ...
        vision=await launch([sys.executable,'-m','scripts.vision.material_shadow_route','--seconds','240','--topic','/research_camera/image','--output',OUT/'vision'],'vision.log',ROOT)
        ...
        phase='postflight'
        await asyncio.wait_for(vision.wait(),245)
