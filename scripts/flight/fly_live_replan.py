...
from scripts.flight import live_replan_fixture as fixture_harness
...
def freeze():
    ...
    sources.append(trial.ROOT/'src/flight/turn_hold.py')
    ...
    async def follow(drone,state,origin):
        ...
        try:
            ...
            while index<len(route)-1:
                ...
                if spawn is None and p['vn']>=.15:
                    event('harness_spawn_requested',measured_speed_m_s=speed)
                    spawn=asyncio.create_task(fixture_harness.insert(OUT))
...
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--fly',action='store_true');p.add_argument('--attempt',type=int,choices=(1,2,3),default=1);args=p.parse_args();OUT=OUT.parent/f'live-replan-turn-hold-{args.attempt:03}'
