...
from scripts.flight import visual_replan_lifecycle as trial
...
from src.sandbox.live_replan_stop import validate_run_id, check_stop
from scripts.flight import retest_sim_heading as prearm
from scripts.flight import check_final_heading as airborne
from scripts.flight import envelope_fixture as fixture_harness
...
GOAL=(3.,1.5)
INITIAL_ROUTE=[(1.5,1.5),GOAL]
MODEL_RUN='material-routed-480-7'
CASE='baseline'

def freeze():
    require_repeat_gate(trial.ROOT)
    world=OUT.parent/'envelope-aware-scene-v1/world.sdf';boxes=collision_boxes(world)
    if not check_route(INITIAL_ROUTE,boxes,map_uncertainty=.20)['passed']:raise ValueError('Initial tracking envelope failed')
    ...
    profile=OUT.parent/'lowload-vehicle-v1/protocol.json'
    profile_record=json.loads(profile.read_text())
    for path,digest in profile_record['inputs'].items():
        if file_sha256(path)!=digest:raise ValueError('Vehicle profile identity changed: '+path)
    ...
    sources.extend([trial.ROOT/'src/flight/turn_hold.py',trial.ROOT/'src/flight/envelope_route.py',trial.ROOT/'src/flight/tracking_envelope.py',trial.ROOT/'src/flight/replan_evidence.py',trial.ROOT/'src/flight/arrival_capture.py',OUT.parent/'envelope-aware-scene-v1/protocol.json',OUT.parent/'startup-repeat-v2/protocol.json',OUT.parent/'startup-repeat-v2/completion.json',profile,trial.ROOT/'src/sensors/gazebo_lidar.py',trial.ROOT/'src/sensors/gazebo_lidar_observed.py',trial.ROOT/'src/sandbox/live_replan_gate.py',trial.ROOT/'src/sandbox/live_replan_stop.py']+[Path(p) for p in profile_record['inputs']])
    sources += [trial.ROOT/'src/flight/startup_gate.py']+[trial.BUILD/'etc/init.d-posix'/name for name in ('rcS','px4-rc.mavlink','px4-rc.simulator')]
    sources += [trial.ROOT/p for p in ('src/flight/sim_pose_sync.py','src/flight/aligned_sim_rgbd.py','src/sensors/gazebo_depth_memory.py','src/flight/visual_replan_service.py','src/flight/visual_target_route.py','src/vision/collection/feedback_recorder.py','scripts/flight/audit_visual_replan.py','src/vision/localization/rgbd_localization.py','src/sensors/gazebo_camera_latest.py','src/sensors/gazebo_camera_memory.py','scripts/flight/check_final_heading.py','scripts/flight/retest_sim_heading.py')]
    from src.sandbox.visual_runtime_model import visual_runtime_model
    info=visual_runtime_model(MODEL_RUN);sources += [info['model_path'],info['receipt_path'],trial.ROOT/'src/sandbox/visual_runtime_model.py']
    write_record(OUT/'protocol.json',dict(simulation_only=True,turn_policy='fixed local position hold; heading setpoint slew 20 deg/s, dt capped 0.1s',goal=GOAL,initial_route=INITIAL_ROUTE,static_boxes=boxes,unregistered_fixture_in_planner=False,fixture_policy='no injected obstacle; semantic target is selected from live YOLO/depth/estimator evidence',map_policy='existing frozen static collision boxes and live LiDAR endpoints only; detector does not read object truth',waypoint_tolerance_m=.03,stop_speed_m_s=.08,stop_stable_s=1.,planning_clearance_m=1.8,lidar_emergency_stop_m=1.5,max_replans=1,route_timeout_s=180,vision_control_authority='bounded_semantic_request',training_admitted=False,promotable=False,inputs={str(p):file_sha256(p) for p in sources}))

async def main():
    protocol=json.loads((OUT/'protocol.json').read_text())
    require_repeat_gate(trial.ROOT)
    validate_run_id(OUT.name)
    if CASE!='baseline':raise ValueError('Sandbox entry supports frozen baseline only')
    check_stop(OUT)
    ...
    try:await trial.main(fly=True,preflight_hook=gate,takeoff_altitude=2.,post_hover_hook=follow,server_config=OUT.parent/'magnetic-contract-001/server.config',post_hover_timeout_s=180.,source_world=OUT.parent/'envelope-aware-scene-v1/world.sdf',visual_service=vision)
    finally:await source.stop()

if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--fly',action='store_true');p.add_argument('--run-id');args=p.parse_args()
    require_repeat_gate(trial.ROOT)
    if not args.fly:
        print('Sandbox repeat gate valid; preflight only, no flight started')
    else:
        validate_run_id(args.run_id);OUT=OUT.parent/args.run_id
        if OUT.exists():raise ValueError('Do not reuse an existing flight attempt')
        freeze()
        with locked_threads(4):asyncio.run(main())
        runtime=json.loads((OUT/'runtime/receipt.json').read_text())
        if runtime['status']!='sitl_hover_landed_disarmed':raise RuntimeError(runtime['error'] or 'Runtime failed')
        from scripts.flight.audit_visual_replan import audit
        audit(OUT)
