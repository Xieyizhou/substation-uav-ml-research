...
def main():
    ...
    paths.append(out/'sensor-diagnostics.json')
    write_record(out/'completion.json',dict(status='sensor_stop_replan_resume_goal_verified' if passed else 'blocked',metrics=metrics,scope='single newly inserted stationary pillar in PX4 SITL; no fixture coordinates supplied to planner; RGB640 flight-only profile, not canonical collection or detector-quality validation',dynamic_object_tracking_verified=False,physical_flight_certified=False,training_admitted=False,promotable=False,inputs={str(p):file_sha256(p) for p in paths}))
