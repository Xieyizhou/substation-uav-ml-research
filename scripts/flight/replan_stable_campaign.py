...
def freeze():
    ...
    units=[dict(case='no_path',repeat=1,directory='replan-stable-v2-no_path-1',expected='safe_rejection')]
    units += [dict(case=c,repeat=n,directory=f'replan-stable-v2-{c}-{n}',expected='goal') for c in ('baseline','west','later') for n in (1,2)]
    sources=[Path(__file__).resolve()]+[ROOT/'scripts/flight'/name for name in ('fly_replan_stable.py','replan_repeat_fixture.py','audit_replan_stable.py','route_trial_lifecycle.py','fly_pillar_route.py','check_final_heading.py','retest_sim_heading.py','prepare_avoidance_route.py')]
...
def run():
    ...
    for unit in protocol['units']:
        ...
        try:
            if not out.exists():
                command=[sys.executable,'-m','scripts.flight.fly_replan_stable','--case',unit['case'],'--repeat',str(unit['repeat'])]
                subprocess.run(command,check=True,cwd=ROOT)
                subprocess.run(command+['--fly'],check=True,cwd=ROOT)
            if not completion.exists():
                replay(out)
                if unit['expected']=='goal':
                    subprocess.run([sys.executable,'-m','scripts.flight.audit_replan_stable',str(out)],check=True,cwd=ROOT)
