...
def freeze():
    ...
    sources.extend([trial.ROOT/'src/flight/turn_hold.py',trial.ROOT/'src/flight/clearance_route.py',trial.ROOT/'src/flight/replan_evidence.py',trial.ROOT/'src/flight/arrival_capture.py',OUT.parent/'replan-capture-v4/protocol.json'])
...
async def main():
    ...
    campaign=json.loads((OUT.parent/'replan-capture-v4/protocol.json').read_text())
...
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--fly',action='store_true');p.add_argument('--case',choices=tuple(fixture_harness.CASES),required=True);p.add_argument('--repeat',type=int,choices=(1,2),required=True);args=p.parse_args();CASE=args.case;OUT=OUT.parent/f'replan-capture-v4-{CASE}-{args.repeat}'
