"""Explicit local SITL-only 1m hover trial; vision never controls flight."""
import asyncio
import argparse
import json
import math
...
OUT=ROOT/'data/research/material-shadow-v1/px4-hover-flight-v1/attempt-001'
...
def guard(state,now,origin,max_height_m=1.8):
    ...
    if height>max_height_m or height<-.4 or drift>1.:raise RuntimeError('Position envelope exceeded')
    if abs(a['roll'])>20 or abs(a['pitch'])>20:raise RuntimeError('Attitude envelope exceeded')
    return height,drift

async def main(fly=False, preflight_hook=None, takeoff_altitude=1., post_hover_hook=None, server_config=None, post_hover_timeout_s=5.):
    if takeoff_altitude not in (1.,2.):raise ValueError('Only bounded 1m or 2m trials supported')
    if not 0<post_hover_timeout_s<=45:raise ValueError('Post-hover timeout must be bounded')
    binary=preflight();config_path=ROOT/'config/substation_obstacles.json';check_spawn(json.loads(config_path.read_text()))
    if shutil.disk_usage(ROOT).free<8*1024**3:raise RuntimeError('Need8GiB free for bounded image evidence')
    OUT.mkdir(parents=True,exist_ok=False);src=ROOT/'simulation/worlds/substation_simple.sdf';world=OUT/'world.sdf'
    prepare_world_with_vehicle(src,world,model_name='x500_research',entity_name='x500_research_0',pose='-8.5,-8.5,0.1,0,0,0')
    ...
    async def wait_until(predicate,timeout,stable=0.,enforce=False):
        ...
        while time.monotonic()<end:
            ...
            if enforce:
                guard(state,time.monotonic(),origin,max_height_m=takeoff_altitude+.8)
    ...
    try:
        ...
        while '/research_camera/image' not in await command('gz','topic','-l'):
            ...
            await asyncio.sleep(1)
        work=OUT/'px4-work';work.mkdir();await launch([binary,BUILD/'etc','-i','7','-d','-w',work,'-s',BUILD/'etc/init.d-posix/rcS'],'px4.log',work)
        ...
        if not fly:
            ...
            return
        vision=await launch([sys.executable,'-m','scripts.vision.material_shadow_latest','--mode','live','--seconds','90','--topic','/research_camera/image','--output',OUT/'vision'],'vision.log',ROOT)
        await wait_until(lambda:(OUT/'vision/detections.jsonl').exists() and (OUT/'vision/detections.jsonl').stat().st_size>0,20)
        ...
        phase='postflight'
        await asyncio.wait_for(vision.wait(),95)
        if vision.returncode:raise RuntimeError('Vision failed')
    except BaseException as e:
        failure=f'{type(e).__name__}: {e}';event('trial_failed',error=failure)
        if drone is not None and armed_requested:
        ...
        paths=[world,src,binary,config_path,OUT/'telemetry.jsonl',Path(__file__).resolve()]
        write_record(OUT/'receipt.json',dict(status='failed' if failure else ('sitl_hover_landed_disarmed' if fly else 'unarmed_health_check_passed'),error=failure,events=events,origin=origin,landing_confirmed=landed,final_armed=state.get('armed',(0,None))[1],simulation_only=True,flight_requested=fly,vision_control_authority='none',owned_processes_exited=all(p.returncode is not None for p in processes),training_admitted=False,promotable=False,inputs={str(p):file_sha256(p) for p in paths}))
    print(failure or 'SITL_HOVER_LANDED_DISARMED',flush=True)

if __name__=='__main__':
    parser=argparse.ArgumentParser();parser.add_argument('--fly',action='store_true');parser.add_argument('--attempt',type=int,choices=(2,3,4),required=True);args=parser.parse_args()
    OUT=OUT.parent/f'attempt-{args.attempt:03}'
    asyncio.run(main(fly=args.fly))
