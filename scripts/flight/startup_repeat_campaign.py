...
def freeze():
    ...
    units=[dict(case='no_path',repeat=1,directory='startup-repeat-v2-no_path-1',expected='safe_rejection')]
    units += [dict(case=c,repeat=n,directory=f'startup-repeat-v2-{c}-{n}',expected='goal') for c in ('baseline','west','later') for n in (1,2)]
    for unit in units:
        if unit['case'] in ('no_path','baseline') or (unit['case'],unit['repeat'])==('west',1):
            unit['directory']=f"lowload-repeat-v1-{unit['case']}-{unit['repeat']}"
            validate_hashes(json.loads((BASE/unit['directory']/'completion.json').read_text()))
    sources=[Path(__file__).resolve()]+[ROOT/'scripts/flight'/name for name in ('fly_startup_replan.py','envelope_fixture.py','audit_lowload_replan.py','startup_trial_lifecycle.py','fly_pillar_route.py','check_final_heading.py','retest_sim_heading.py','prepare_avoidance_route.py','check_px4_shadow.py','measure_mapping_error.py')]
    ...
    sources += [ROOT/'src/flight/startup_gate.py',BASE/'lowload-repeat-v1/completion.json',BASE/'lowload-repeat-v1-west-2/runtime/receipt.json']
    for n in (1,2):
        probe=BASE/f'px4-startup-probe-v1-{n}/completion.json';record=json.loads(probe.read_text());validate_hashes(record)
        if record['status']!='unarmed_startup_verified' or not record['owned_processes_exited']:raise ValueError('Startup probe incomplete')
        sources.append(probe)
    from scripts.flight.check_px4_shadow import BUILD
    sources += [BUILD/'etc/init.d-posix'/name for name in ('rcS','px4-rc.mavlink','px4-rc.simulator')]
    ...
    write_record(OUT/'protocol.json',dict(version='startup-repeat-v2',resource_preflight_wait_s=60,change='explicit unarmed PX4 startup gate and shell trace; unchanged flight controller, lidar, RGB640, safety guards and envelope; reuse three audited positive flights and one negative from lowload v1, preserve failed pre-arm attempt; run only remaining three units',cases=CASES,units=units,harness_feasibility=feasibility,positive_required=6,negative_required=1,max_attempts_per_unit=1,stop_on_any_unexpected_result=True,pass_rule='all six complete sensor stop/replan/resume/goal and audited landing; negative trial reproduces no path or blocked goal and safely lands; all hashes valid',scope='PX4 SITL only; stationary newly inserted pillar; no physical flight',sandbox_enable_requires_all=True,training_admitted=False,promotable=False,inputs={str(p):file_sha256(p) for p in sources}))
            ...
            if not out.exists():
                ...
                while True:
                    try:preflight();break
                        ...
                        time.sleep(1)
                command=[sys.executable,'-m','scripts.flight.fly_startup_replan','--case',unit['case'],'--repeat',str(unit['repeat'])]
                subprocess.run(command,check=True,cwd=ROOT)
                subprocess.run(command+['--fly'],check=True,cwd=ROOT)
            if not completion.exists():
                runtime=json.loads((out/'runtime/receipt.json').read_text())
                if not (out/'scan-pose.jsonl').exists():raise ValueError('No flight sensor evidence; runtime: '+str(runtime.get('error')))
                replay(out)
