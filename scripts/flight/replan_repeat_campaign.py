...
def freeze():
    ...
    units=[dict(case='no_path',repeat=1,directory='replan-repeat-v1-no_path-1',expected='safe_rejection')]
    units += [dict(case=c,repeat=n,directory=f'replan-repeat-v1-{c}-{n}',expected='goal') for c in ('baseline','west','later') for n in (1,2)]
    sources=[Path(__file__).resolve()]+[ROOT/'scripts/flight'/name for name in ('fly_replan_repeat.py','replan_repeat_fixture.py','audit_live_replan.py','route_trial_lifecycle.py','fly_pillar_route.py','check_final_heading.py','retest_sim_heading.py','prepare_avoidance_route.py')]
    sources += [ROOT/'src/flight/live_obstacle_map.py',ROOT/'src/flight/turn_hold.py',BASE/'live-replan-turn-hold-002/completion.json']
    OUT.mkdir(exist_ok=False)
    write_record(OUT/'protocol.json',dict(version='replan-repeat-v1',cases=CASES,units=units,harness_feasibility=feasibility,positive_required=6,negative_required=1,max_attempts_per_unit=1,stop_on_any_unexpected_result=True,pass_rule='all six complete sensor stop/replan/resume/goal and audited landing; no-path trial reproduces no path and safely lands; all hashes valid',scope='PX4 SITL only; stationary newly inserted pillar; no physical flight',sandbox_enable_requires_all=True,training_admitted=False,promotable=False,inputs={str(p):file_sha256(p) for p in sources}))
...
def audit_rejection(out):
    ...
    files=[out/'protocol.json',out/'route.json',out/'runtime/receipt.json',mp,out/'pillar.sdf',out/'pillar-receipt.json',Path(__file__).resolve()]
...
def run():
    ...
    for unit in protocol['units']:
        ...
        try:
            if not out.exists():
                command=[sys.executable,'-m','scripts.flight.fly_replan_repeat','--case',unit['case'],'--repeat',str(unit['repeat'])]
                subprocess.run(command,check=True,cwd=ROOT)
                subprocess.run(command+['--fly'],check=True,cwd=ROOT)
            if not completion.exists():
                if unit['expected']=='goal':
                    subprocess.run([sys.executable,'-m','scripts.flight.audit_live_replan',str(out)],check=True,cwd=ROOT)
    ...
    files=[OUT/'protocol.json']+[BASE/u['directory']/'completion.json' for u in results if u['passed']]
