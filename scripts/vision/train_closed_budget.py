        ...
        for i in range(0,len(KEYS),parallel):
            ...
            try:
                for key in KEYS[i:i+parallel]:
                    ...
                    proc=subprocess.Popen([sys.executable,'-u','-m','scripts.vision.train_closed_budget','--worker',key,'--train'],cwd=prior.ROOT,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
