...
OUT=DATA/'compensated-exposure-feasibility-v1'
...
def run():
    ...
    cr=COMP/'reviewed-completion.json';review=prior.read(cr);prior.verify(review)
    if review['status']!='one_compensating_source_verified_not_training_ready':raise ValueError('Compensator not reviewed')
    source=review['sources'][0]
    candidates.append(dict(member_id='C01-original',class_instances=dict(Counter(x['class_name'] for x in source['full_truth']['objects']))))
    ...
    OUT.mkdir(exist_ok=True);results={};paths=[manifest,DATA/'reviewed-completion.json',cr,solver_design,sf,Path(__file__)]
    for seed in (7,17,27):
        ...
        lo=np.array([0]*n+[3]*12+[1],dtype=float);hi=np.array([old[x['member_id']] for x in oldrows]+[540]*k,dtype=float)
        ...
        record=dict(seed=seed,solver_status=int(result.status),message=result.message,
            scope='Only remove original bridge exposures and insert new poses; no old member increases. Twelve material poses >=3; C01 >=1 is an additional common original-appearance member for both V and VM.',
    ...
    prior.frozen(OUT/'result.json',dict(results=results,
        ...
        objective='Minimize inserted exposure count with original twelve poses >=3 and C01 >=1; common C01 member must be identical between V and VM. Feasibility diagnostic only.',
