...
from scripts.vision.prepare_visibility_repair_training import OUT as INITIAL_OUT,ROOT,KEYS,read,save,file_sha256,verify_tree,baseline_verify
import scripts.vision.prepare_visibility_repair_training as preparation
OUT=INITIAL_OUT/'epoch-initialization-v2'
def prepare():
    preparation.OUT=OUT
...
def run_worker(key):
    ...
    try:
        with (attempt/'worker.log').open('x') as log:
            proc=subprocess.Popen([sys.executable,'-u','-m','scripts.vision.run_visibility_repair_training_v2','--worker',key],cwd=ROOT,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
