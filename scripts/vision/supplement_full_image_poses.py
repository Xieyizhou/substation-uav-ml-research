...
OUT=PRIOR/'full-image-pose-screen-supplement-v1'
...
def screen(view,objects,hfov=1.466,width=1920,height=1080):
    ...
    for obj in objects:
        ...
        tx=math.tan(hfov/2);ty=tx*height/width
        planes=[lambda p:p[0]-.1,lambda p:120-p[0],lambda p:p[0]*tx-p[1],lambda p:p[0]*tx+p[1],lambda p:p[0]*ty-p[2],lambda p:p[0]*ty+p[2]]
        if any(all(plane(p)<0 for p in corners) for plane in planes):continue
...
def main():
    ...
    old=PRIOR/'full-image-pose-screen-v2/screen.json';inputs[str(old)]=file_sha256(old)
    ...
    selected=read(old)['selected']
    for category in ('switchgear',):
        ...
        for r in options:
            ...
            chosen.append(r)
            if len(chosen)+sum(x['view']['category']==category for x in selected)==2:break
        selected.extend(chosen);counts['added:'+category]=len(chosen)
    for p in (PRIOR/'handoff.json',SOURCE,CONFIG,SOURCE.parent/'world.sdf',Path(__file__),ROOT/'tests/test_full_image_pose_screen_v2.py'):inputs[str(p)]=file_sha256(p)
    OUT.mkdir(parents=True,exist_ok=True)
    save(path,dict(status='geometric_candidates_ready_for_original_only_pilot' if len(selected)==8 else 'bounded_screen_coverage_gap',rows=rows,selected=selected,counts=dict(counts),rounds=[405,406,407],scope='Retain seven frozen v2 candidates; add at most one switchgear from rounds 405-407 under identical geometry rules. Raw originals first. No appearance expansion before all full-image labels are visually reviewed. AABB frustum and center-ray checks are conservative prefilters, not masks or proof of visibility. A zero-candidate result does not prove no physically feasible view exists.',inputs=inputs))
