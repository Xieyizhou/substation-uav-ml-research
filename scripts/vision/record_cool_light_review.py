...
from scripts.vision.cool_light_observations import NOTES
...
def main():
    ...
    deps = [OUT/'capture-protocol.json', OUT/'condition-audit.json', OUT/'pose-role-audit.json',
            Path(__file__).resolve(), Path(__file__).with_name('cool_light_observations.py')]
    decisions, coverage = [], []
    for u in p['units']:
        uid = u['unit_id']
        ...
        if c['status'] != 'low_light_capture_exact_replay_verified' or r['status'] != 'original_pixel_evidence_certified':
    ...
    r = prior.frozen(dest, dict(status='32_frames_explicitly_reviewed_exact_replay_complete',
        decisions=decisions, full_frames_viewed=list(NOTES), coverage=coverage,
        background_source_check=dict(frames=['L21','L22','L23','L24'],
        ...
        limitations=['Some neutral-material top boundaries have low contrast against the floor; dark faces and major body content remain identifiable.',
                    ...
                    '32 derivatives of eight training poses with shared assets, using an already viewed development lighting recipe.',
                    'Exact replay certifies each new capture, not pixel equality between different lighting conditions.',
                    'Inherited card heading lower_light is display text; actual cool-light XML and condition audit determine configuration.'],
