...
KEYS=tuple(f'{arm}-{s}' for s in (7,17,27) for arm in ('fixed','multiscale'))

def contract(key):
    if key not in KEYS:raise ValueError('Unknown training cell')
    p=prior.read(OUT/'protocol.json');prior.verify(p)
    c=prior.read(OUT/'completion.json');prior.verify(c)
    if c['status']!='loader_preflight_passed_training_not_started':raise ValueError('Loader gate incomplete')
    source=prior.read(p['source_protocol']);prior.verify(source)
    import torch,ultralytics
    if dict(torch=torch.__version__,ultralytics=ultralytics.__version__)!=p['environment']:raise ValueError('Environment changed')
    init=source['initialization']
    if prior.file_sha256(init['path'])!=init['sha256']:raise ValueError('Initialization changed')
    arm,seed=key.rsplit('-',1);found=list((OUT/'checks'/f'seed-{seed}').glob('attempt-*/complete.json'))
    if len(found)!=1:raise ValueError('Missing/duplicate preflight')
    r=prior.read(found[0]);validate(r,p,source);paired_checks(r)
    cfg=source['training_config'][f'R-clean-{seed}']
    if cfg!=p['design']['configs'][seed] or cfg['multi_scale']!=0 or cfg['imgsz']!=640 or cfg['lr0']!=.0005:raise ValueError('Config drift')
    return p,source,r['arms'][arm]
...
class BatchGate:
    ...
    def apply(self,owner,batch):
        ...
        e=self.expected['batches'][self.step];members=[self.lookup[x] for x in batch['im_file']]
        if members!=e['members'] or tensor_hash(batch['img'])!=e['source640_sha256']:raise ValueError('Member/brightness/base tensor changed')
        for f,sha in e['full_supervision'].items():
            if tensor_hash(batch[f])!=sha:raise ValueError('Complete supervision changed')
        out=preprocess(owner,batch,e['size'])
        if list(out['img'].shape)!=e['effective_shape'] or tensor_hash(out['img'])!=e['effective_tensor_sha256']:raise ValueError('Effective training tensor differs from preflight')
        for f,sha in e['full_supervision'].items():
...
def ready():
    ...
    if r['status']!='training_entry_verified_not_started' or r['cells']!=list(KEYS):raise ValueError('Entry not ready')
...
def complete(key):
    ...
    if x['actual']!=e['actual'] or x['batch_records']!=e['batches'] or x['brightness_log']!=e['brightness_log']:raise ValueError('Actual training inputs changed')
...
def worker(key):
    ...
    try:
        ...
        torch.set_num_threads(4);seed=int(key.rsplit('-',1)[1]);sk=f'R-clean-{seed}'
        ...
        for key in KEYS:
            ...
            try:
                with (attempt/'log.txt').open('x') as log:
                    proc=subprocess.Popen([sys.executable,'-u','-m','scripts.vision.train_frozen_multiscale','--train','--worker',key],cwd=prior.ROOT,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
