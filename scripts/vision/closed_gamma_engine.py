...
def execute(p,key,attempt,threads,expected,probe_steps=None):
    ...
    logs=[];gamma_logs=[];actual=[];records=[];steps=[];batch_started=[];lookup={r['image_path']:r['member_id'] for r in p['pool_rows']}
    ...
    class Trainer(DetectionTrainer):
        ...
        def get_dataloader(self,dataset_path,batch_size=16,rank=0,mode='train'):
            ...
            return make_loader(make_dataset(p,key,logs,gamma_logs),p,key,self)
    ...
    if probe_steps:
        ...
        if actual!=expected['actual'][:target*6] or logs!=expected['brightness_log'][:target*6] or gamma_logs!=expected['gamma_log'][:target*6]:raise ValueError('Probe input drift')
    else:
        check(p,key,actual,logs,gamma_logs)
        if logs!=expected['brightness_log'] or gamma_logs!=expected['gamma_log']:raise ValueError('Full augmentation bytes changed')
    from src.ml.artifacts import object_sha256
    state_identity=object_sha256({n:tensor_hash(t) for n,t in model.trainer.model.state_dict().items()})
    return dict(actual=actual,batch_records=records,brightness_log=logs,gamma_log=gamma_logs,optimizer_steps=len(steps),step_records=steps,
