...
import numpy as np
from scripts.vision.design_compensating_pose_pilot import OUT as DESIGN,freeze,prior
...
async def run():
    p=freeze()
    if p['status']!='one_compensation_source_frozen' or len(p['selected'])!=1:raise ValueError('Invalid single-source design')
    ...
    if not pp.exists():prior.frozen(pp,dict(status='one_compensation_source_frozen',views=p['selected'],plan_path=p['plan_path'],
    ...
    with (OUT/'runner.lock').open('a') as lock:
        ...
        prior.frozen(dest,dict(status='one_compensation_pose_checked_review_required',units=rows,training_ready=False,training_started=False,inputs={str(p):prior.file_sha256(p) for p in paths}))
