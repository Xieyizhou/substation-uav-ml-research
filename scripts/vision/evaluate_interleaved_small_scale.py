"""Fixed endpoint evaluation of order-only controls; explicit review remains separate."""
import argparse
import fcntl
import signal
import subprocess
...
def evaluate(key):
    ...
    path=OUT/'evaluation'/f'{key}-order-binding.json'
    if path.exists(): prior.verify(prior.read(path))
        ...
        prior.frozen(path,dict(status='order_adapter_bound',inputs={str(d):prior.file_sha256(d) for d in deps}))
    return r


def finish():
    records={k:evaluate(k) for k in KEYS}; p=freeze()
    deps=[OUT/'design.json',Path(__file__).resolve()]
    deps += [OUT/'evaluation'/f'{k}{s}.json' for k in KEYS for s in ('','-order-binding')]
    old={}
    for key in source.KEYS:
        ep=source.OUT/'evaluation'/f'{key}.json'; old[key]=prior.read(ep); prior.verify(old[key]); deps.append(ep)
    refs=[fixed.PRIOR/f'evaluation-retained_reference-450-{s}.json' for s in (7,17,27)]
    reference=[prior.read(x) for x in refs]
    hp=Path(p['evaluation']['historical_reference']); h=prior.read(hp)
    for r in reference+[h]: prior.verify(r)
    deps += refs+[hp]
    groups={f:fixed.aggregate([records[f'{f}-{s}'] for s in (7,17,27)]) for f in ('ISR1100','ISM1100')}
    original={f:fixed.aggregate([old[f'{f}-{s}'] for s in (7,17,27)]) for f in ('SR1100','SM1100')}
    ...
    return prior.frozen(dest,dict(status='numerical_complete_explicit_error_review_pending',groups=groups,blocked_order_controls=original,
        policies={f:fixed.policy_checks(g,fixed.aggregate(reference),h['historical_A'],policy) for f,g in groups.items()},
        direct_changes={f:fixed.direct_checks(g,original[f[1:]]) for f,g in groups.items()},
        planned_paired_changes={f:{str(s):fixed.paired_change(old[f'{f[1:]}-{s}']['rows'],records[f'{f}-{s}']['rows']) for s in (7,17,27)} for f in groups},
        matching_conflicts=sum(r['matching_conflicts'] for r in records.values()),baseline=baseline,
        review_complete=False,selected_candidate=None,inputs={str(d):prior.file_sha256(d) for d in deps}))
        ...
        for start in range(0,len(KEYS),2):
            ...
            try:
                for k in KEYS[start:start+2]:
                    ...
                    proc=subprocess.Popen([sys.executable,'-u','-m','scripts.vision.evaluate_interleaved_small_scale','--evaluate','--cell',k],
...
if __name__=='__main__':
    ...
    else: freeze(); print('PREFLIGHT_ONLY_NO_INFERENCE')
