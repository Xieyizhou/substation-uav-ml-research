...
OUT=ORIGINAL/'appearance-expansion-v1'
MATRIX={'materials':{'steel':'0.22 0.26 0.30 1'},'lights':{'warm_dim':{'ambient':'0.25 0.22 0.18 1','sun_diffuse':'0.55 0.48 0.38 1'}}}
...
def prepare():
    ...
    for variant in ('steel_original','steel_warm_dim'):
    ...
    paths=[review,audit,sourcepath,rp,Path(__file__),Path(__file__).with_name('capture_appearance_recovery_pilot.py')]+[Path(x['plan_path']) for x in runs]
    return save(target,dict(status='frozen_16_frame_expansion',runs=runs,matrix=MATRIX,lineage=lineage,
      original_frames=8,new_frames=16,source_groups=7,map_count=1,
...
async def main():
    ...
    with (OUT/'capture.lock').open('a') as lock:
        ...
        for run in p['runs']:
            ...
            save(OUT/'progress.json',dict(status='complete_pending_AI_review' if len(runs)==2 and all(x['captured']==8 for x in runs) else 'incomplete',runs=runs,
