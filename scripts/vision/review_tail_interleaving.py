...
from scripts.vision.evaluate_tail_interleaving import OUT,SOURCE,prior,KEYS,complete
...
def build():
    ...
    for key in KEYS:
        ...
        for family,base in [('R1000',SOURCE),('B900',SOURCE.parent)]:
            ...
            for row in r['rows']:
                ...
                for j,t in enumerate(row['truth']):
                    ...
                    e=dict(comparison=family+'->I1000',seed=seed,pair_id=row['pair_id'],variant=row['variant'],object_id=src['object_id'],truth=t,state=state,miss=next((m for m in row['misses'] if m['truth_index']==j),None),low_predictions=row['low_predictions'])
