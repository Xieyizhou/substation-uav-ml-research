...
from scripts.vision.freeze_gray_body_control import OUT,prior,freeze
from scripts.vision.finalize_gray_body_route import check_receipt
...
KEYS=tuple(f'G-{s}' for s in (7,17,27))

def contract(key):
    if key not in KEYS:raise ValueError('Unknown cell')
    p=freeze()
    final=prior.read(OUT/'route-ready.json');prior.verify(final)
    if final['status']!='ready_for_bounded_condition_transfer_training_not_started':raise ValueError('Route not ready')
    prior.verify(prior.read(OUT/'loader-completion.json'))
    seed=int(key.rsplit('-',1)[1]);_,reference,_=reference_contract(f'fixed-{seed}')
    if p['training_config'][key]!=reference['training_config'][f'R-clean-{seed}']:raise ValueError('Configuration changed')
    if p['initialization']!=reference['initialization'] or p['evaluation']!=reference['evaluation']:raise ValueError('Init/evaluation drift')
    if prior.file_sha256(p['initialization']['path'])!=p['initialization']['sha256']:raise ValueError('Weight changed')
    ...
    r=prior.read(files[0]);prior.verify(r);check_receipt(p,r,seed)
    import copy
    expected=copy.deepcopy(r['cells'][key])
    for batch in expected['batch_records']:
        batch['image_tensor_sha256']=batch.pop('tensor_sha256')
        batch['full_supervision']=batch.pop('supervision')
    return p,p,expected
...
def run():
    ...
    with (OUT/'training.lock').open('a') as lock:
        ...
        for key in KEYS:
            ...
            try:
                with (attempt/'log.txt').open('x') as log:
                    proc=subprocess.Popen([sys.executable,'-u','-m','scripts.vision.train_gray_body_control','--train','--worker',key],cwd=prior.ROOT,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
        ...
        prior.frozen(OUT/'training/completion.json',dict(status='three_cells_trained_not_evaluated',cells=list(KEYS),inputs={str(OUT/'training'/k/'completion.json'):prior.file_sha256(OUT/'training'/k/'completion.json') for k in KEYS}))
