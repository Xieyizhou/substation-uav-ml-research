...
import numpy as np
from scripts.vision.design_supplemental_full_scene_poses import OUT as DESIGN,freeze,prior
...
async def run():
    ...
    if not pp.exists():prior.frozen(pp,dict(status='eight_fixed_supplemental_views_frozen',views=p['selected'],plan_path=p['plan_path'],
    ...
    with (OUT/'runner.lock').open('a') as lock:
        ...
        prior.frozen(dest,dict(status='eight_supplemental_pose_checks_finished_review_required',units=rows,training_ready=False,training_started=False,inputs={str(p):prior.file_sha256(p) for p in paths}))
