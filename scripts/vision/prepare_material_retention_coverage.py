"""Export 24 already reviewed target/all-body variants for bounded controls, without admission."""
...
prior=pilot.prior
OUT=DESIGN/'material-retention-coverage-v1'
...
def main():
    ...
    for module in (pilot,expansion):
        ...
        for page in e['pages']:
            ...
            members.append(dict(member_id=sid+'-'+condition,source_pose_id=sid,pair_id=source['pair_id'],
                planned_object_id=source['target_object_id'],variant=condition,full_truth=t,
            ...
            paths += [tp,rp]
    if len(members)!=24 or len({m['source_pose_id'] for m in members})!=12:raise ValueError('Incomplete reviewed body sets')
    ...
    else:prior.frozen(review_path,dict(status='twenty_four_reviewed_body_variants_pending_export',members=members,
