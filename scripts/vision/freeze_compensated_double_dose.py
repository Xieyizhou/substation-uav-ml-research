"""Bounded fixed-budget 54->108 new-member dose design; never trains."""
import argparse
import hashlib
import sys
...
OUT=DEST/'reviewed-double-dose-control-v1'
VERSION='reviewed-compensated-double-dose-v1'
KEYS=source.KEYS
...
def checks(old,p):
    ...
    for key in KEYS:
        ...
        for m,r in rows.items():
            if 'full_truth' in r:
                if b[m]!=2*a[m]:raise ValueError('New member dose not exactly twofold')
...
def solve(old,seed,milp,Bounds,LinearConstraint):
    ...
    extra={m:n for m,n in counts.items() if 'full_truth' in rows[m]}
    ...
    lower=np.zeros(n+1);upper=np.array([counts[r['member_id']]-1 for r in selected]+[54],dtype=float)
    ...
    return dict(status='exact_twofold_dose_witness',seed=seed,removed={r['member_id']:int(v) for r,v in zip(selected,x[:-1]) if v},
        extra_original_members=extra,additional_positions=54,max_old_member_decrement=optimum,
...
def schedules(old,seed,witness):
    ...
    if len(items)!=len(pos) or len(pos)!=54:raise ValueError('Position mismatch')
    oldvm=Counter(vm);remaining={m:n for m,n in oldvm.items() if 'full_truth' in rows[m]};used=Counter()
...
def freeze():
    ...
    paths=[DEST/'reviewed-dose-control-v1/counts.json',source.OUT/'protocol.json',source.OUT.parent/'dataset-completion.json',FIT/'protocol.json',FIT/'summary.json',DEST/'review-summary.json',Path(__file__).resolve()]
    ...
    cp=OUT/'counts.json';prior.frozen(cp,dict(status='counts_checked',results=result,training_started=False,
        ...
        multiplier=2,new_image_exposures=108,total_image_exposures=2700,
        inputs={str(p):prior.file_sha256(p) for p in paths}));paths.append(cp)
    if any(w['status']!='exact_twofold_dose_witness' for w in result.values()):raise ValueError('Twofold fixed-budget design not feasible; no automatic relaxation')
    ...
    checks(old,p)
    p.update(status='dose_sequences_frozen_preflight_pending',new_member_multiplier=2,
        ...
        limits=['Shared layout/assets; C01 and S07 concentration ratios unchanged','Old bridge exposure decreases; this is part of the strategy',
                'Member/brightness combinations change at 54 additional positions','Twofold is a bounded test, not a proven sufficient exposure threshold',
