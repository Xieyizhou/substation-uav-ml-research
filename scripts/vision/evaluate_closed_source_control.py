...
from scripts.vision.train_closed_source_control import OUT, KEYS, complete, contract, prior
...
def evaluate(key):
    ...
    try:
        ...
        paths = [OUT/'protocol.json', cp, rp, np, Path(__file__).resolve(), Path(cell['weights']),BENCH/'completion.json']
...
def finish():
    ...
    groups={family:aggregate([records[f'{family}-{seed}'] for seed in (7,17,27)]) for family in ('E','M')}
    ...
    return prior.frozen(dest,dict(status='numerical_complete_explicit_error_review_pending',
        ...
        paired_changes={str(seed):paired_change(records[f'E-{seed}']['rows'],records[f'M-{seed}']['rows']) for seed in (7,17,27)},
        matching_conflicts=sum(r['matching_conflicts'] for r in records.values()),baseline=baseline,selected_candidate=None,
        comparison='E vs M: same eight new sources, identical full supervision and 30 replacement slots; M retains eight original exposures and uses 22 material exposures. Historical T changes sources and instance exposure, background reference only.',
