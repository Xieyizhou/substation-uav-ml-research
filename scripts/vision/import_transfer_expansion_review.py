...
from scripts.vision.build_transfer_expansion_review import OUT, prior

DEST = OUT / 'review'
OBS = Path(__file__).with_name('transfer_expansion_observations.json')
...
def main():
    ...
    return prior.frozen(rp, dict(status='all_label_observations_recorded_not_training_admitted',
        decisions=decisions, unique_frames=len(e['pages']), independent_source_poses=8,
