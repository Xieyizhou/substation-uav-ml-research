...
from scripts.vision.neutral_gray_capture import OUT,SOURCE,COHORT,freeze,prior
from scripts.vision.verify_neutral_gray import verify_capture,evidence
...
from scripts.vision.cool_light_control import freeze as base_design
...
def validate_review(p,r):
    ...
    for d in ds:
        e,x=want[d['event_id']]
        if d['evidence_identity']!=e['identity'] or d['crop_sha256']!=x['crop_sha256'] or d['image_sha256']!=e['image_sha256']:raise ValueError('Stale explicit review')
        if d['object_id']!=x['object_id'] or not d['review_time']:raise ValueError('Review identity/time missing')
...
def export():
    ...
    old=base_design();refs=list(old['pool_rows']);
    from scripts.vision.physical_low_light_capture import OUT as NEUTRAL
    neutral=prior.read(NEUTRAL/'export/manifest.json');prior.verify(neutral);refs+=neutral['members'];deps=[NEUTRAL/'export/manifest.json',OUT/'capture-protocol.json',rp,SOURCE/'pool-fit-diagnosis-v1/summary.json',SOURCE/'pose-role-audit.json',COHORT/'quality-review.json',Path(__file__).resolve()]
    ...
    for u in p['units']:
        ...
        if replay['status']!='gray_capture_exact_replay_verified':raise ValueError('Replay not certified')
        ...
        seen.add(px);member='gray035-'+u['source_member']['member_id'];image=OUT/'export/images'/f'{member}.png';label=OUT/'export/labels'/f'{member}.txt'
        ...
        members.append(dict(u['source_member'],member_id=member,source_member_id=u['source_member']['member_id'],illumination=u['source_member'].get('illumination','original'),material_setting='neutral_0.35',image_path=str(image),image_sha256=prior.file_sha256(image),pixel_sha256=px,
    ...
    return prior.frozen(dest,dict(status='16_reviewed_lossless_counterparts_isolation_checked',members=members,reference_images_checked=len(refs),protected_labels_read=False,
        independent_pose_groups=8,new_independent_pose_groups=0,layouts=2,shared_assets=True,derivation_policy='Each gray-material counterpart stays in the same training role and lineage as its original. No development or protected image used for training.',inputs={str(p):prior.file_sha256(p) for p in deps}))
