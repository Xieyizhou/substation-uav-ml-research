...
from scripts.vision import evaluate_closed_gamma as evaluation
...
def run():
    ...
    with (root/'run.lock').open('a') as lock:
        ...
        for i in range(0,len(keys),parallel):
            ...
            try:
                for key in keys[i:i+parallel]:
                    ...
                    proc=subprocess.Popen([sys.executable,'-u','-m','scripts.vision.evaluate_closed_gamma','--evaluate','--cell',key],cwd=prior.ROOT,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
        ...
        prior.frozen(dest,dict(status='three_gamma_models_evaluated_error_review_pending',wall_seconds=time.monotonic()-start,
