...
OUT=SOURCE/'physical-low-light-control-v1'
LIGHT={'.//world/scene/ambient':'0.42 0.42 0.42 1','.//world/light[@name="sun"]/diffuse':'0.60 0.60 0.58 1'}
...
def freeze():
    ...
    feedback();manifest=export();prior.verify(manifest)
    ...
    OUT.mkdir(exist_ok=True);deps=[GAMMA/'evaluation-completion.json',COHORT/'export/manifest.json',COHORT/'quality-review.json',Path(__file__).resolve()];units=[]
    for n,m in enumerate(rows,1):
        ...
        p.update(calibration_views=[v],pilot_views=[],diagnostic_purpose='physical_low_light_training_counterpart',data_role='bounded_development_training_candidate',training_admitted=False,promotable=False)
    ...
    return prior.frozen(dest,dict(status='32_lower_light_candidates_frozen_quality_pending',units=units,pilot_units=pilot,lighting=LIGHT,
        ...
        limitations=['Eight already-reviewed training pose groups, two layouts, existing shared assets. No new independent scene from relighting.','Low light is a fixed neutral lower-intensity renderer condition, not the development lighting recipe.','Appended stage budget effect requires paired reference; comparison tests this data/stage strategy, not unique cause.','No protected labels, sealed scenes, historical edits or promotion.'],
