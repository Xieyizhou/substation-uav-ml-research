...
from scripts.vision.evaluate_physical_low_light import OUT,KEYS,prior,validate_record
from scripts.vision.closed_gamma_design import SOURCE
from concurrent.futures import ProcessPoolExecutor
import time
from scripts.vision.material_control_feasibility import OUT as IDENTITY
...
def main():
    ...
    src=IDENTITY/'initial-gate.json'
    ...
    for seed in (7,17,27):
        key=f'B900-{seed}';path=SOURCE/'evaluation'/f'{key}.json';r=prior.read(path);validate_record(r,key);records[key]=r;paths.append(path)
    identities={}
    for r in identity['corrected_target_records']:
        ...
        identities[k]=r
    transitions=[];losses={}
    for left,right in (('B900','R1000'),('B900','L1000'),('R1000','L1000')):
        for seed in (7,17,27):
            a={(r['pair_id'],r['variant']):r for r in records[f'{left}-{seed}']['rows']}
            for b in records[f'{right}-{seed}']['rows']:
                ...
                for j,t in enumerate(b['truth']):
                    ...
                    best={}
                    for name,row in ((left,old),(right,b)):
                    ...
                    item=dict(comparison=left+'->'+right,seed=seed,pair_id=b['pair_id'],variant=b['variant'],object_id=source['object_id'],truth=t,state=state,miss=miss,best_matching_same_class=best)
                    transitions.append(item)
                    if state=='loss':
                        loss=losses.setdefault(k,dict(kind='LOSS',source=source,truth=t,events=[]))
                        loss['events'].append(item)
    _,p,_=__import__('scripts.vision.evaluate_physical_low_light',fromlist=['contract']).contract(KEYS[0])
    ...
    for key in KEYS:
        ...
        for r in records[key]['negative_rows']:
            for prediction in r['predictions']:
                ...
                fp['events'].append(dict(cell=key,seed=seed,prediction=prediction))
    events=list(fps.values())+list(losses.values())
    for n,x in enumerate(events,1):x['event_id']=f'E{n:02}'
    start=time.monotonic()
    with ProcessPoolExecutor(max_workers=2) as pool:events=list(pool.map(render,events))
    render_seconds=time.monotonic()-start
    for x in events:paths.extend([Path(x['source']['image_path']),Path(x['page_path'])])
    pages=[]
    ...
    counts=Counter(f"{x['comparison']}:{x['variant']}:{x['state']}" for x in transitions)
    prior.frozen(DEST/'evidence.json',dict(status='awaiting_explicit_visual_review',events=events,transitions=transitions,pages=pages,
        preprocessing_workers=2,render_seconds=render_seconds,inference_repeated=False,
        transition_counts=dict(counts),inputs={str(p):prior.file_sha256(p) for p in paths}))
    print('EVENTS',len(events),'FP frames',len(fps),'LOSS boxes',len(losses),'PAGES',len(pages),'RENDER_SECONDS',render_seconds)
...
def render(x):
        ...
        cp=DEST/f"{x['event_id']}.png";card.save(cp)
        x['page_path']=str(cp);x['page_sha256']=prior.file_sha256(cp);return x
