"""Evidence pages and own-label crops for eight newly rendered variants."""
...
def main():
    ...
    if c['status']!='eight_variants_captured_review_pending':raise ValueError('Incomplete variant matrix')
    ...
    prior.frozen(dest/'evidence.json',dict(status='24_explicit_label_reviews_required',events=events,training_ready=False,inputs={str(p):prior.file_sha256(p) for p in paths}))
    print('PAGES8 LABELS',len(events))
