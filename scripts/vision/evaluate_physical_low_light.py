...
from scripts.vision.train_physical_low_light import OUT, KEYS, complete, ready, prior
def contract(key):
...
from scripts.vision import train_closed_budget as direct_train
...
def finish():
    ...
    for seed in (7,17,27):
        oldkey=f'B900-{seed}';direct_train.complete(oldkey);ep=direct_train.OUT/'evaluation'/f'{oldkey}.json'
    ...
    groups={family:aggregate([records[f'{family}-{seed}'] for seed in (7,17,27)]) for family in ('R1000','L1000')}
    ...
    return prior.frozen(dest,dict(status='numerical_complete_explicit_error_review_pending',
        ...
        paired_changes={f'{left}->{right}':{str(seed):paired_change((direct[str(seed)] if left=='B900' else records[f'{left}-{seed}'])['rows'],records[f'{right}-{seed}']['rows']) for seed in (7,17,27)} for left,right in (('B900','R1000'),('B900','L1000'),('R1000','L1000'))},
        matching_conflicts=sum(r['matching_conflicts'] for r in records.values()),baseline=baseline,selected_candidate=None,
        comparison='R1000 vs L1000: identical 900-step prefix and paired full supervision, source lineage and tail batch order; 300 of 600 tail exposures change to physical low light. B900 comparisons also change budget and source rehearsal. Three seeds do not increase independent scenes.',
