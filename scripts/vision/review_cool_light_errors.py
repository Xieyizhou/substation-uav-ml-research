...
from scripts.vision.cool_light_control import OUT,REF,prior,KEYS
from scripts.vision.evaluate_cool_light import configure,adapter
configure()
...
def build():
    ...
    if (DEST/'evidence.json').exists():
        r=prior.read(DEST/'evidence.json');prior.verify(r);return r
        ...
        for family,base in [('IL1000',REF)]:
            ...
            for row in r['rows']:
                ...
                for j,t in enumerate(row['truth']):
                    ...
                    e=dict(comparison=family+'->IC1000',seed=seed,pair_id=row['pair_id'],variant=row['variant'],object_id=src['object_id'],truth=t,state=state,miss=next((m for m in row['misses'] if m['truth_index']==j),None),low_predictions=row['low_predictions'])
