...
from scripts.vision.train_closed_budget import OUT as TRAIN, KEYS, ready, complete, prior, cleanup
from scripts.vision.closed_budget_design import SOURCE
def contract(key):
    if key not in KEYS:raise ValueError('Unknown budget')
...
def freeze():
    ...
    deps=[TRAIN/'design.json',SOURCE/'cpu-inference-policy-lock.json',Path(__file__).resolve(),TRAIN/'evaluation-completion.json']
    for dep in deps:
        if dep.suffix=='.json':prior.verify(prior.read(dep))
    ...
    for key in KEYS:
        ...
        if sum(exposures[key].values())!=len(source['schedules'][key]):raise ValueError('Exposure count')
    ...
    for i in range(0,len(KEYS),2):
        ...
        try:
            for key in KEYS[i:i+2]:
                ...
                proc=subprocess.Popen([sys.executable,'-u','-m','scripts.vision.diagnose_closed_budget_fit','--worker',key],start_new_session=True,cwd=prior.ROOT)
