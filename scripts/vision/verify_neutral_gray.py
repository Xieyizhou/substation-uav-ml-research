...
from scripts.vision.neutral_gray_capture import OUT,freeze,prior,material_only
...
def verify_capture(u):
    ...
    material_only(ET.parse(Path(u['source_plan']).parent/'world.sdf').getroot(),ET.parse(pp.parent/'world.sdf').getroot())
...
def evidence(u):
    ...
    cd.text((5,5),u['unit_id']+' '+u['source_member']['pair_id']+' '+u['source_member']['variant']+' neutral_gray_035',fill='black');card.paste(full.resize((960,540)),(0,35))
    refim=Image.open(u['source_member']['image_path']).convert('RGB');refim.thumbnail((470,300));card.paste(refim,(965,35));cd.text((965,15),'source .42 material / same lighting',fill='black')
...
async def replay_unit(u):
    ...
    if not r or r['status']!='original_pixel_evidence_certified' or any(x['missing_targets'] for x in r['full_mask_coverage']):raise ValueError('Gray replay blocked '+u['unit_id']+' '+str(r.get('reason') if r else 'missing'))
    dest=unit/'completion.json'
    if not dest.exists():prior.frozen(dest,dict(status='gray_capture_exact_replay_verified',receipt_path=str(rp),inputs={str(x):prior.file_sha256(x) for x in (rp,pp)}))
