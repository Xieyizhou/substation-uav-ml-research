...
from scripts.vision.expand_transfer_probe_v2 import OUT, prior
...
def main():
    ...
    if c['status']!='captured_explicit_review_pending' or len(c['units'])!=40:raise ValueError('Incomplete expansion')
    ...
    for u in c['units']:
        ...
        paths += [ip,ppng,rp,tp]
    return prior.frozen(ep,dict(status='explicit_all_label_expansion_review_pending',events=events,pages=pages,
