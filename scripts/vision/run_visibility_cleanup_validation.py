...
from scripts.vision.instance_visibility_diagnosis import (
    OUT as PREVIOUS, ROOT, PILOT, prepare as prepare_source, read, save, file_sha256, verify_tree, ET,
...
OUT=PREVIOUS/'cleanup-validated-replay-v1'
...
def prepare():
    ...
    if path.exists():
        ...
        return read(path)
    old=prepare_source()
    verify_tree(PREVIOUS/'handoff.json')
    OUT.mkdir(parents=True,exist_ok=True)
    import subprocess,shlex
    flags=subprocess.check_output(['pkg-config','--cflags','--libs','gz-transport13','gz-msgs10'],text=True)
    source=ROOT/'tools/gz_visibility_capture_cleanup_fixed.cc'
    subprocess.run(['clang++',str(source),'-o',str(OUT/'gz_visibility_capture_cleanup_fixed'),*shlex.split(flags)],check=True)
    return save(path,dict(frames=old['frames'],pilot=PILOT,policy=old['policy'],lineage_closure=old['lineage_closure'],
        status='frozen',authorization='User authorized autonomous repairs, collection and training toward unchanged quality gates; this phase verifies cleanup and visibility only.',
        max_attempts=3,previous_failures_preserved=True,
        inputs={str(p):file_sha256(p) for p in (PREVIOUS/'handoff.json',PREVIOUS/'protocol.json',Path(__file__),source,OUT/'gz_visibility_capture_cleanup_fixed')}))
...
async def attempt(frame, number):
    ...
    try:
        ...
        with (folder/'simulator.log').open('x') as log:
            ...
            await command(str(OUT/'gz_visibility_capture_cleanup_fixed'),str(folder),frame['world_name'],env=env,timeout=70)
            result=analyze(folder,frame,fence)
            result['move_fence_timestamp']=fence
    ...
    for p in (OUT/'protocol.json',Path(__file__),ROOT/'tools/gz_visibility_capture_cleanup_fixed.cc',OUT/'gz_visibility_capture_cleanup_fixed'):
    ...
    async def unit(frame):
        ...
        for n in range(1,4):
            ...
            if rp.exists():
                ...
                if r['status'] not in ('technical_failure','topics_available'):return r
                continue
