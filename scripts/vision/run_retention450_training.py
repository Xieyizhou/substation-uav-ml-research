...
from scripts.vision.prepare_retention450_training import OUT,ROOT,KEYS,read,save,file_sha256,verify_tree,baseline_verify,prepare,validate_args
...
def prepare_execution():
    ...
    modules=['test_visibility_repair_design','test_visibility_repair_full_labels','test_visibility_repair_runtime','test_retained_bridge_review','test_canonical_shutdown','test_retention_real_quota','test_retention_regression_review','test_retention450_adapter']
...
def worker(key):
    ...
    class CheckedLoader(OriginalLoader):
        def __init__(self,*args,**kwargs):
            ...
            t=sampler.trainer;epoch=getattr(t,'epoch',0);observed=[]
            try:
...
def run_worker(key):
    ...
    try:
        with (attempt/'worker.log').open('x') as log:
            proc=subprocess.Popen([sys.executable,'-u','-m','scripts.vision.run_retention450_training','--worker',key],cwd=ROOT,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--worker',choices=KEYS);ap.add_argument('--prepare-only',action='store_true');ap.add_argument('--train',action='store_true');args=ap.parse_args()
    if args.worker:worker(args.worker);return
    OUT.mkdir(parents=True,exist_ok=True)
    with (OUT/'runner.lock').open('a') as lock:
        fcntl.flock(lock,fcntl.LOCK_EX|fcntl.LOCK_NB);prepare_execution()
        if not args.train or args.prepare_only:print('EXECUTION_READY_NO_TRAINING_STARTED');return
        completed=[]
        for key in KEYS:
