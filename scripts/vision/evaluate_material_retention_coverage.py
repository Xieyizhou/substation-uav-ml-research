...
from scripts.vision.train_material_retention_coverage import OUT, KEYS, complete, contract, prior
...
from scripts.vision import train_gray_body_control as gray
...
def finish():
    ...
    groups={a:aggregate([records[f'{a}-{s}'] for s in (7,17,27)]) for a in ('T','A')}
    d=aggregate(list(direct.values()));gray_records={}
    for seed in (7,17,27):
        key=f'G-{seed}';gray.complete(key);ep=gray.OUT/'evaluation'/f'{key}.json'
        rec=prior.read(ep);validate_record(rec,key)
        c=prior.read(gray.OUT/'training'/key/'completion.json')
        if rec['inputs'].get(c['weights'])!=c['weights_sha256']:raise ValueError('Gray reference weight mismatch')
        gray_records[str(seed)]=rec;paths.append(ep)
    ...
    paths.append(OUT/'decision-protocol.json')
    ...
    return prior.frozen(dest,dict(status='numerical_complete_explicit_error_review_pending',
        aggregate=groups,direct_reference_aggregate=d,gray_reference_aggregate=aggregate(list(gray_records.values())),
        policy_results={a:policy_checks(g,aggregate(reference),historical['historical_A'],policy) for a,g in groups.items()},
        direct_retention={a:direct_checks(g,d) for a,g in groups.items()},
        color_retention=direct_checks(groups['T'],aggregate(list(gray_records.values()))),
        scope_retention=direct_checks(groups['A'],groups['T']),
        paired_changes={a:{s:paired_change(direct[s]['rows'],records[f'{a}-{s}']['rows']) for s in direct} for a in groups},
        color_changes={s:paired_change(gray_records[s]['rows'],records[f'T-{s}']['rows']) for s in direct},
        scope_changes={s:paired_change(records[f'T-{s}']['rows'],records[f'A-{s}']['rows']) for s in direct},
        matching_conflicts=sum(r['matching_conflicts'] for r in records.values()),selected_candidate=None,baseline=baseline,
        comparison='Color retention T versus historical G; gray coverage A versus T; VM retained as reference. Fixed budgets.',
