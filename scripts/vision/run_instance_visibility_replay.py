...
async def attempt(frame, number):
    ...
    result=dict(status='technical_failure');server=None;env=dict(os.environ,GZ_PARTITION='visibility-'+uuid.uuid4().hex,GZ_IP='127.0.0.1')
    try:
        ...
        with (folder/'simulator.log').open('x') as log:
            ...
            await command(str(OUT/'gz_visibility_capture'),str(folder),frame['world_name'],env=env,timeout=40)
            result=analyze(folder,frame,fence)
    except ValueError as e:result=dict(status='semantic_blocked',reason=str(e))
    except (Exception,asyncio.CancelledError) as e:result=dict(status='technical_failure',reason=f'{type(e).__name__}: {e}')
    finally:await stop_group(server)
    inputs={str(p):file_sha256(p) for p in folder.iterdir() if p.is_file()}
    for p in (OUT/'protocol.json',Path(__file__),ROOT/'tools/gz_visibility_capture.cc',OUT/'gz_visibility_capture'):
