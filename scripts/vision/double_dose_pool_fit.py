...
from scripts.vision import train_compensated_double_dose as trained
from scripts.vision import train_compensated_material as previous
...
OUT = trained.OUT/'evaluation/pool-fit-v1'
KEYS = tuple(f'{a}-{s}' for s in (7,17,27) for a in ('V','VM'))
...
def freeze():
    ...
    paths = [trained.OUT/'protocol.json', previous.OUT.parent/'dataset-completion.json',
