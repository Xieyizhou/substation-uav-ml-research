...
def complete(key):
    ...
    if x['batch_records']!=expected['batch_records'] or x['brightness_log']!=expected['brightness_log']:raise ValueError('Actual batch mismatch')
...
def run():
    ...
    with (OUT/'training.lock').open('a') as lock:
        ...
        else:prior.frozen(auth,dict(status='explicit_goal_authorizes_all_six_training_units',cells=list(KEYS),inputs={str(OUT/'entry-ready.json'):prior.file_sha256(OUT/'entry-ready.json')}))
        for i in range(0,len(KEYS),parallel):
            ...
            try:
                for key in KEYS[i:i+parallel]:
                    ...
                    proc=subprocess.Popen([sys.executable,'-u','-m','scripts.vision.train_physical_low_light','--worker',key,'--train'],cwd=prior.ROOT,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
        ...
        if not dest.exists():prior.frozen(dest,dict(status='all_six_trained_evaluation_pending',cells=list(KEYS),inputs={str(OUT/'training'/k/'completion.json'):prior.file_sha256(OUT/'training'/k/'completion.json') for k in KEYS}))
