...
def evaluate(key):
    ...
    path=OUT/'evaluation'/f'{key}-gray-binding.json'
...
def finish():
    if (OUT/'evaluation/summary.json').exists():
        r=prior.read(OUT/'evaluation/summary.json');prior.verify(r);return r
    ...
    for key in KEYS:
        rp=REF/'evaluation'/('IC1000-'+key.split('-')[-1]+'.json');r=prior.read(rp);prior.verify(r);refs.append(r)
        deps.extend([rp,OUT/'evaluation'/f'{key}.json',OUT/'evaluation'/f'{key}-gray-binding.json'])
    ...
    return prior.frozen(OUT/'evaluation/summary.json',dict(status='numerical_complete_explicit_review_pending',aggregate=g,IC1000=ref,B900=old['B900'],
        policy=fixed.policy_checks(g,fixed.aggregate(historical),h['historical_A'],policy),relative_IC1000=fixed.direct_checks(g,ref),relative_B900=fixed.direct_checks(g,old['B900']),
        ...
        for group in (KEYS[:2],KEYS[2:]):
            ...
            try:
                for key in group:
                    ...
                    proc=subprocess.Popen([sys.executable,'-u','-m','scripts.vision.evaluate_neutral_gray','--cell',key,'--evaluate'],cwd=prior.ROOT,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
