...
from scripts.vision.train_closed_gamma import OUT, KEYS, complete, ready, prior
def contract(key):
...
def finish():
    ...
    groups={family:aggregate([records[f'{family}-{seed}'] for seed in (7,17,27)]) for family in ('G900',)}
    ...
    return prior.frozen(dest,dict(status='numerical_complete_explicit_error_review_pending',
        ...
        paired_changes={str(seed):paired_change(direct[str(seed)]['rows'],records[f'G900-{seed}']['rows']) for seed in (7,17,27)},
        matching_conflicts=sum(r['matching_conflicts'] for r in records.values()),baseline=baseline,selected_candidate=None,
        comparison='B900 vs G900: identical 5400 member exposures, full supervision, base brightness, initialization and optimization settings; only frozen Gamma differs. Three seeds are repeated fits, not independent new scenes.',
