...
KEYS=tuple(f'{a}-{s}' for a in ('T','A') for s in (7,17,27))

def contract(key):
    if key not in KEYS:raise ValueError('Unknown cell')
    decision=prior.read(OUT/'decision-protocol.json');prior.verify(decision)
    if decision['cells']!=list(KEYS):raise ValueError('Decision cells changed')
    p=freeze()
    final=prior.read(OUT/'route-ready.json');prior.verify(final)
    if final['status']!='ready_for_material_retention_coverage_training_not_started':raise ValueError('Route not ready')
    prior.verify(prior.read(OUT/'loader-completion.json'))
    seed=int(key.rsplit('-',1)[1]);_,reference,_=reference_contract(f'fixed-{seed}')
    if decision['acceptance_policy']!=reference['acceptance_policy'] or decision['retention']!=reference['retention']:raise ValueError('Frozen evaluation gate drift')
        ...
        for key in KEYS:
            ...
            try:
                with (attempt/'log.txt').open('x') as log:
                    proc=subprocess.Popen([sys.executable,'-u','-m','scripts.vision.train_material_retention_coverage','--train','--worker',key],cwd=prior.ROOT,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
