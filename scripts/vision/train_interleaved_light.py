...
from scripts.vision.interleaved_light_control import OUT,REF,KEYS,freeze,prior
        ...
        for group in (KEYS[:2],KEYS[2:]):
            ...
            try:
                for key in group:
                    ...
                    proc=subprocess.Popen([sys.executable,'-u','-m','scripts.vision.train_interleaved_light','--worker',key,'--train'],cwd=prior.ROOT,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
