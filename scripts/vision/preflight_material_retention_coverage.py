...
def run():
    ...
    for seed in (7,17,27):
        ...
        try:
            ...
            keys=[f'R-{seed}',f'T-{seed}',f'A-{seed}'];logs={k:[] for k in keys};actual={k:[] for k in keys};batches={k:[] for k in keys}
            ...
            with ExitStack() as stack:
                ...
                for epoch in range(45):
                    owner.epoch=epoch
                    for triple in zip(*loaders,strict=True):
                        a=triple[0]
                        for b in triple[1:]:
                        ...
                        for key,batch in zip(keys,triple,strict=True):
            ...
            affected={x['position'] for x in p['treatment_positions'][str(seed)]}
            for other in keys[1:]:
                for i,(a,b) in enumerate(zip(logs[keys[0]],logs[other],strict=True)):
                    if a['gain']!=b['gain']:raise ValueError('Gain changed')
                    if i not in affected and a!=b:raise ValueError('Non-treatment exposure drift')
            dest=attempt/'complete.json'
            prior.frozen(dest,dict(status='actual_three_arm_loaders_verified',seed=seed,
            ...
            print('COMPLETE',seed,'8100 actual loads',flush=True)
    ...
    prior.frozen(dest,dict(status='nine_actual_loaders_verified_final_audit_pending',training_ready=False,training_started=False,
