...
def isolated_render(args):
    event, directory = args
    with patch.object(builder, 'DEST', Path(directory)):
        return builder.render(dict(event))

def main():
    dest=DEST/'diagnostic-confirmation-v2.json'
    ...
    with TemporaryDirectory(prefix='closed-review-render-') as tmp:
        ...
        with patch.object(builder,'DEST',Path(tmp)):
            ...
            with ProcessPoolExecutor(max_workers=2) as pool:
                parallel=list(pool.map(isolated_render,[(x,tmp) for x in e['events']]))
    ...
    return prior.frozen(dest,dict(status='review_complete_with_named_content_limits_no_candidate',
        ...
        supersedes='diagnostic-confirmation.json: benchmark worker output isolation was invalid; original cards restored to exact frozen hashes; use v2 timings only.',
