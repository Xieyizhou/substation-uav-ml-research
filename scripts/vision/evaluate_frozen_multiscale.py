...
from scripts.vision.train_frozen_multiscale import OUT, KEYS, complete, contract, prior
...
def finish():
    ...
    groups = {a:aggregate([records[f'{a}-{s}'] for s in (7,17,27)]) for a in ('fixed','multiscale')}
    ...
    return prior.frozen(OUT/'evaluation/summary.json',dict(status='numerical_complete_explicit_error_review_pending',aggregate=groups,
        policy_results={a:policy_checks(g,aggregate(reference),historical['historical_A'],p) for a,g in groups.items()},
        direct_retention=direct_checks(groups['multiscale'],groups['fixed']),
        paired_changes={str(s):paired_change(records[f'fixed-{s}']['rows'],records[f'multiscale-{s}']['rows']) for s in (7,17,27)},
