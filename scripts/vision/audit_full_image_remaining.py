...
def main():
    ...
    prior_review=PRIOR/'review/decisions.json';verify_tree(prior_review);inputs[str(prior_review)]=file_sha256(prior_review)
    originals=read(ORIGINAL/'review/decisions.json')['frames']+read(prior_review)['frames'];allframes=originals+r['frames']
    ...
    checks=[]
    for f in fingerprints[len(originals):]:
