...
from scripts.vision import train_compensated_material as historical
...
KEYS=tuple(f'{arm}-{s}' for s in (7,17,27) for arm in ('V','VM'))

def contract(key):
    if key not in KEYS:raise ValueError('Unknown cell')
    p=prior.read(OUT/'protocol.json');prior.verify(p)
    checks(prior.read(historical.OUT/'protocol.json'),p)
    final=prior.read(historical.OUT.parent/'dataset-completion.json');prior.verify(final)
    diagnosis=prior.read(OUT/'decision-protocol.json');prior.verify(diagnosis)
    if diagnosis['status']!='frozen_bounded_double_dose_comparison':raise ValueError('Diagnosis/decision protocol invalid')
    if final['status']!='ready_for_bounded_development_training_not_started':raise ValueError('Dataset not ready')
    ...
    r=prior.read(files[0]);prior.verify(r);validate(p,r,seed)
    return p,p,r['cells'][key]
        ...
        for key in KEYS:
            ...
            try:
                with (attempt/'log.txt').open('x') as log:
                    proc=subprocess.Popen([sys.executable,'-u','-m','scripts.vision.train_compensated_double_dose','--train','--worker',key],cwd=prior.ROOT,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
