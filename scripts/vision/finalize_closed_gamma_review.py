...
from scripts.vision.closed_gamma_design import OUT
...
def main():
    ...
    for t in rs['loss_details']:
        ...
        ep=OUT/'evaluation'/f"G900-{t['seed']}.json";p=prior.read(ep);prior.verify(p);deps.append(ep)
    ...
    return prior.frozen(dest,dict(status='evaluation_and_AI_review_complete_with_named_gaps_no_candidate',
        ...
        planned_assignment_conflicts=s['matching_conflicts'],policy_passed=s['policy_results']['G900']['passed'],selected_candidate=None,
        conclusion='Reject this frozen Gamma strategy; do not infer all augmentation fails. Clear-body and partial-content losses coexist. Development unknowns remain counted, not training members.',
