...
from scripts.vision.evaluate_compensated_material import OUT,KEYS,prior,validate_record,REFERENCE_OUT
...
DEST=OUT.parent/'next-pretraining-diagnosis-v1'
...
def main():
    ...
    for seed in (7,17,27):
        path=REFERENCE_OUT/'evaluation'/f'fixed-{seed}.json'
        r=prior.read(path);validate_record(r,f'fixed-{seed}');records[f'R-{seed}']=r;paths.append(path)
    identities={}
    for r in identity['corrected_target_records']:
        ...
        identities[k]=r
    transitions=[];losses={}
    for left,right in (('R','V'),('R','VM'),('V','VM')):
        for seed in (7,17,27):
    ...
    _,p,_=__import__('scripts.vision.train_compensated_material',fromlist=['contract']).contract(KEYS[0])
