"""Explicit cool-recipe training; default contract only, CPU 4 threads per worker."""
...
def contract():
    ...
    deps=[OUT/'design.json',OUT/'quality-review.json',OUT/'condition-audit.json',fp,Path(p['initialization']['path']),Path(__file__).resolve(),Path(engine.__file__).resolve(),Path(__file__).with_name('closed_budget_engine_v2.py')]
    return p,f,deps,b
        ...
        for group in (KEYS[:2],KEYS[2:]):
            ...
            try:
                for key in group:
                    ...
                    proc=subprocess.Popen([sys.executable,'-u','-m','scripts.vision.train_cool_light','--worker',key,'--train'],cwd=prior.ROOT,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
