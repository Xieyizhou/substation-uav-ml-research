"""Eight body-material variants from four verified sources; no training."""
import argparse,asyncio,copy,fcntl,json,os,shutil
from pathlib import Path
import xml.etree.ElementTree as ET
from scripts.vision.verify_designed_full_scene_poses import OUT as SOURCES,DESIGN,prior
...
def freeze():
    ...
    if review['status']!='four_new_sources_verified_material_variants_not_started':raise ValueError('Sources not ready')
    ...
    return prior.frozen(dest,dict(status='eight_variants_frozen',units=rows,helper=str(helper),max_attempts=3,
...
async def run():
    ...
    with (OUT/'runner.lock').open('a') as lock:
        ...
        prior.frozen(dest,dict(status='eight_variants_captured_review_pending' if len(results)==8 and not blocked else 'incomplete_triplets_held',units=results,
            training_ready=False,training_started=False,inputs={str(p):prior.file_sha256(p) for p in paths}))
