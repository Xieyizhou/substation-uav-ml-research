...
BASE=PARENT/'full-scene-pose-design-v1'
OUT=PARENT/'full-scene-supplement-v1'
...
def freeze():
    ...
    bp=BASE/'real-verification-v1/reviewed-completion.json';br=prior.read(bp);prior.verify(br);paths.append(bp)
    seen += [r['actual_pose'] for r in br['sources']]
    records=[];eligible=[]
    for obj in source['objects']:
        if obj['category'] not in CLASSES:continue
        center=[sum(obj['bounds'][i:i+2])/2 for i in (0,2,4)]
        for bearing,distance,height in itertools.product([7.5+15*i for i in range(24)],(5.,6.,7.5,8.,10.,12.,14.,15.,16.,18.),(3.,5.,7.,9.,11.)):
    ...
    for category in ('capacitor_bank','switchgear','reactor','transformer'):
        ...
        if len(rows)>=2:
            # Fixed near/far extremes within new feasible grid, not model/visual ranking.
            selected.append(dict(rows[0],probe_id='S'+str(len(selected)+1).zfill(2)))
            selected.append(dict(rows[-1],probe_id='S'+str(len(selected)+1).zfill(2)))
    ...
    return prior.frozen(dest,dict(status='bounded_pose_plan_frozen' if not missing else 'missing_classes_no_expansion',
        ...
        policy='Eight fixed supplemental representatives (two per class) before real capture; full target AABB inside 16px margin and all in-frustum target center rays unblocked. Margin is a conservative experimental prefilter, not a training quality threshold. Projection is not a mask or label. Heights 3/5/7/9/11m are offline camera experiments, not UAV flight certification.',
