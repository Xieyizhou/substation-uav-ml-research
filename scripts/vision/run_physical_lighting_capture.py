...
def analyze(folder,frame,fence):
    ...
    for n in indices:
        ...
        stamps=[base.message_timestamp(v) for v in meta.values()];skew=(max(stamps)-min(stamps))*1000
        ...
        records.append(dict(capture_index=n,rgb_exact_vs_original=exact,skew_ms=skew,full_boxes=boxes,visible_boxes=visible,
