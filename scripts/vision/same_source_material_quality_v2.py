...
def main():
    ...
    for row in selected:
        ...
        for truth,obj in zip(src['full_truth']['objects'],resolved,strict=True):
            if variant=='gray_target_body':
                ...
                source_member=originals[mid]
                source_image=Path(source_member['image_path'])
                if prior.file_sha256(source_image)!=source_member['image_sha256']:raise ValueError('Source RGB hash changed')
                with Image.open(source_image) as a,Image.open(row['image_path']) as b:
                    if a.size!=b.size or a.convert('RGB').tobytes()!=b.convert('RGB').tobytes():raise ValueError('Original RGB pixels differ')
                paths.append(source_image)
                candidates=[(d,review) for d in legacy[review]['decisions'] if d.get('image_sha256')==source_member['image_sha256'] and d.get('truth')==truth and d.get('object_id')==obj['object_id']]
            if len(candidates)!=1:
    ...
    OUT.mkdir(exist_ok=True);dest=OUT/'quality-v2.json'
