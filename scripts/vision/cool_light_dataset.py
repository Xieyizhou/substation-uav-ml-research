...
from scripts.vision.cool_light_capture import OUT,COHORT,freeze,prior
from scripts.vision.verify_cool_light import verify_capture,evidence
...
def export():
    ...
    neutral=prior.read(NEUTRAL/'export/manifest.json');prior.verify(neutral);refs+=neutral['members'];deps=[NEUTRAL/'export/manifest.json',OUT/'capture-protocol.json',rp,OUT/'condition-audit.json',OUT/'pose-role-audit.json',COHORT/'quality-review.json',Path(__file__).resolve()]
    ...
    for u in p['units']:
        ...
        seen.add(px);member='cool-'+u['source_member']['member_id'];image=OUT/'export/images'/f'{member}.png';label=OUT/'export/labels'/f'{member}.txt'
        ...
        members.append(dict(u['source_member'],member_id=member,source_member_id=u['source_member']['member_id'],illumination='physical_cool_viewed_development_recipe',image_path=str(image),image_sha256=prior.file_sha256(image),pixel_sha256=px,
    ...
    return prior.frozen(dest,dict(status='32_reviewed_lossless_counterparts_isolation_checked',members=members,reference_images_checked=len(refs),protected_labels_read=False,
        independent_pose_groups=8,new_independent_pose_groups=0,layouts=2,shared_assets=True,derivation_policy='Each cool-light counterpart stays in the same training role and lineage as its original. No development or protected image used for training.',inputs={str(p):prior.file_sha256(p) for p in deps}))
