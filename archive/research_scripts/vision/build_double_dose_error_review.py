...
from scripts.vision.evaluate_compensated_double_dose import OUT,KEYS,prior,validate_record,low_dose
...
def main():
    ...
    for key in KEYS:
        path=low_dose.OUT/'evaluation'/f'{key}.json'
        r=prior.read(path);validate_record(r,key);records[f'L-{key}']=r;paths.append(path)
    identities={}
    for r in identity['corrected_target_records']:
        ...
        identities[k]=r
    transitions=[];losses={}
    for left,right in (('L-V','V'),('L-VM','VM'),('V','VM')):
        for seed in (7,17,27):
    ...
    _,p,_=__import__('scripts.vision.train_compensated_double_dose',fromlist=['contract']).contract(KEYS[0])
