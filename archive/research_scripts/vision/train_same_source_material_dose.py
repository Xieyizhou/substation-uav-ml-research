...
from scripts.vision.preflight_same_source_material_dose import OUT,KEYS,protocol
        ...
        for key in KEYS:
            ...
            try:
                with (attempt/'log.txt').open('x') as log:
                    proc=subprocess.Popen([sys.executable,'-u','-m','scripts.vision.train_same_source_material_dose','--train','--worker',key],cwd=prior.ROOT,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
