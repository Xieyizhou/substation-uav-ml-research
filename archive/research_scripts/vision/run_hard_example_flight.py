...
def run(args):
    ...
    try:
        ...
        if args.visual_route is None:
            ...
            command = [str(ROOT / ".venv/bin/python"), "scripts/flight/run_task.py", "run", "fly_round_trip", "--", "--obstacle-config", str(obstacle), "--visual-route", str(args.visual_route), "--return-home", "--visual-mission-events", str(events)]
        ...
        try:
            ...
            stage = "blocked"
            error = f"{error or ''}; flight cleanup failed: {cleanup_error}"
        safe_to_stop = flight is None or any((row.get("event") or row.get("event_type")) == "landing_confirmed" for row in _events(events))
        if safe_to_stop:
            _terminate_group(launcher)
        else:
            stage = "blocked"
            error = f"{error or ''}; landing unconfirmed: simulator retained for recovery"
        if safe_to_stop and owned_launcher_pid is not None:
