"""Run one frozen E/F composition cell and verify every actual exposure."""
import argparse
import json
import sys
...
def train(arm,seed):
    ...
    if seed not in protocol["seeds"] or arm not in protocol["arms"]: raise ValueError("Unknown training cell")
    ...
    member_rows={row["member_id"]:row for row in protocol["pool_rows"]}
    member_ids=protocol["membership"]["member_ids"]
    paths=[member_rows[mid]["image_path"] for mid in member_ids]
    path_for_member={mid:member_rows[mid]["image_path"] for mid in member_ids}
    schedule=protocol["schedules"][arm][str(seed)]
    expected=[[path_for_member[mid] for mid in epoch] for epoch in schedule];actual=[]
    ...
    class Trainer(DetectionTrainer):
        ...
        def get_dataloader(self,dataset_path,batch_size=16,rank=0,mode="train"):
            ...
            return DataLoader(dataset,batch_size=controls["batch"],sampler=ScheduledSampler(self,mapping),num_workers=0,collate_fn=dataset.collate_fn,generator=torch.Generator().manual_seed(seed))
    ...
    model.train(trainer=Trainer,data=str(dataset_path),epochs=controls["epochs"],imgsz=controls["input_size"],
        ...
        lr0=controls["lr0"],lrf=controls["lrf"],warmup_epochs=0,warmup_bias_lr=0,seed=seed,deterministic=True,
        patience=0,amp=False,mosaic=0,close_mosaic=0,mixup=0,copy_paste=0,degrees=0,translate=0,scale=0,
        shear=0,perspective=0,flipud=0,fliplr=0,hsv_h=0,hsv_s=0,hsv_v=0,project=str(OUT),name=run_name,
        plots=False,save=True,val=False)
    ...
    result={"status":"complete","arm":arm,"seed":seed,"protocol_identity":protocol["identity"],
        "optimizer_steps":model.trainer.step_count,"observed_draws":sum(len(x["paths"]) for x in actual),
        "exposure_verified":True,"weights":str(weights),"weights_sha256":file_sha256(weights),
        "ultralytics":ultralytics.__version__,"torch":torch.__version__,"training_admitted":False,"promotable":False}
    result["identity"]=object_sha256(result);write_json(completion,result);return result


if __name__=="__main__":
    parser=argparse.ArgumentParser();parser.add_argument("--arm",choices=list("EF"),required=True);parser.add_argument("--seed",type=int,choices=[7,17,27],required=True);args=parser.parse_args()
