...
from scripts.vision import evaluate_closed_source_control as evaluation
...
def run():
    ...
    keys=[f'{family}-{seed}' for seed in (7,17,27) for family in ('E','M')]
        ...
        for i in range(0,len(keys),parallel):
            ...
            try:
                for key in keys[i:i+parallel]:
                    ...
                    proc=subprocess.Popen([sys.executable,'-u','-m','scripts.vision.evaluate_closed_source_control','--evaluate','--cell',key],cwd=prior.ROOT,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
