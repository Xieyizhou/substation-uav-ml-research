...
from scripts.vision.train_material_late_rehearsal import OUT, KEYS, complete, contract, prior
...
def finish():
    ...
    for seed in (7,17,27):
        key=f'L-{seed}';complete(key);ep=OUT/'evaluation'/f'{key}.json'
    ...
    return prior.frozen(dest,dict(status='numerical_complete_explicit_error_review_pending',
        ...
        comparison='L versus T: fixed-multiset source-level late material timing strategy.',
