"""Capture the six-frame positive bridge pilot, stopping on first failed run."""
import asyncio
import json
import sys
...
from scripts.vision.prepare_visual_bridge_supplement import BASE,prepare


async def main():
    matrix=prepare();runs=[]
    for spec in matrix["positive_pilot"]["runs"]:
        out=Path(spec["plan_path"]).parent.parent/"capture"
        receipt=await collect(spec["plan_path"],out,mode="calibration")
