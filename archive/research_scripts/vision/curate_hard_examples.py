...
def main():
    ...
    strict_duplicates = protocol.get("deduplicate_within_split", False)
    if not isinstance(strict_duplicates, bool):
        raise ValueError("deduplicate_within_split must be a boolean")
    if strict_duplicates and "class_frame_quotas" not in protocol:
        raise ValueError("strict duplicate selection requires multilabel quotas")
    ...
    if "class_frame_quotas" in protocol:
        ...
        selected, duplicate_rejections, clusters, coverage, shortfall = curate_multilabel(
            ...
            deduplicate_within_split=strict_duplicates,
        )
    else:
        quotas = {
