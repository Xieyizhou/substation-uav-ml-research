...
from scripts.vision import train_material_retention_coverage as historical
from scripts.vision.freeze_material_learning_trajectory import OUT, KEYS, main as freeze_trajectory
from scripts.vision.material_trajectory_runtime import capture
def contract(key):
    if key not in KEYS: raise ValueError('Unknown trajectory cell')
    trajectory=freeze_trajectory()
    p,source,expected=historical.contract(key)
    if trajectory['cells'][key]['source_sequence']!=expected['actual']: raise ValueError('Trajectory input drift')
...
def ready():
    trajectory=freeze_trajectory()
    receipt=prior.read(OUT/'entry-ready.json');prior.verify(receipt)
    if receipt['status']!='trajectory_entry_verified_not_started' or receipt['protocol_identity']!=trajectory['identity']:
        raise ValueError('Trajectory preflight missing or stale')
    for key in KEYS: historical.complete(key);contract(key)
...
def complete(key):
    ...
    snapshots=prior.read(r['snapshot_receipt']);prior.verify(snapshots)
    if [x['step'] for x in snapshots['checkpoints']]!=freeze_trajectory()['cells'][key]['checkpoint_steps']:
        raise ValueError('Missing trajectory checkpoint')
    import torch
    h=prior.read(historical.OUT/'training'/key/'completion.json')
    a=torch.load(r['weights'],map_location='cpu',weights_only=False)
    b=torch.load(h['weights'],map_location='cpu',weights_only=False)
    a=(a.get('ema') or a['model']).state_dict();b=(b.get('ema') or b['model']).state_dict()
    if set(a)!=set(b) or any(not torch.equal(a[k],b[k]) for k in a):
        raise ValueError('Terminal reproduction conflict: investigate before interpretation')
...
def worker(key):
    ...
    try:
        ...
        snapshots=[];trajectory=freeze_trajectory();snapshot_steps=set(trajectory['cells'][key]['checkpoint_steps'])
        ...
        class Trainer(DetectionTrainer):
            ...
            def optimizer_step(self):
                ...
                result=super().optimizer_step();self.step_count+=1
                if self.step_count in snapshot_steps:
                    snapshots.append(capture(self,attempt/'checkpoints'/f'step-{self.step_count:03}.pt',self.step_count))
                return result
        ...
        model.add_callback('on_train_start',lambda trainer:snapshots.append(capture(trainer,attempt/'checkpoints/step-000.pt',0)))
        model.train(trainer=Trainer,data=str(data),project=str(attempt),name='run',**source['training_config'][sk])
        if [x['step'] for x in snapshots]!=sorted(snapshot_steps):raise ValueError('Checkpoint coverage incomplete')
        snapshot_receipt=attempt/'checkpoints.json'
        prior.frozen(snapshot_receipt,dict(checkpoints=snapshots,inputs={x['path']:x['sha256'] for x in snapshots}))
        ...
        paths=[snapshot_receipt,OUT/'entry-ready.json',OUT/'protocol.json',OUT/'training-authorization.json',Path(__file__).resolve(),xp,weight,data,args,curve]
        prior.frozen(root/'completion.json',dict(status='complete',cell=key,protocol_identity=p['identity'],optimizer_steps=450,
            weights=str(weight),weights_sha256=prior.file_sha256(weight),exposure_path=str(xp),snapshot_receipt=str(snapshot_receipt),checkpoint_selection='terminal_last_only',
...
def run():
    ...
    with (OUT/'training.lock').open('a') as lock:
        ...
        for key in KEYS:
            ...
            try:
                with (attempt/'log.txt').open('x') as log:
                    proc=subprocess.Popen([sys.executable,'-u','-m','scripts.vision.train_material_learning_trajectory','--train','--worker',key],cwd=prior.ROOT,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
        ...
        prior.frozen(OUT/'training/completion.json',dict(status='three_trajectory_cells_trained_not_evaluated',cells=list(KEYS),inputs={str(OUT/'training'/k/'completion.json'):prior.file_sha256(OUT/'training'/k/'completion.json') for k in KEYS}))
