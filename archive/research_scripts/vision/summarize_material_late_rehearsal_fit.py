"""Matched T/L member fit and frozen historical loss-cohort diagnosis."""
    ...
    for seed in (7,17,27):
        ...
        for condition in sorted({m['condition'] for m in p['members']}):
            ...
            counts.append(dict(seed=seed,condition=condition,target_count=len(target),
                T_target_hits=sum(x['before_hit'] for x in target),L_target_hits=sum(x['after_hit'] for x in target),
                full_truth_count=len(group),T_full_hits=sum(x['before_hit'] for x in group),L_full_hits=sum(x['after_hit'] for x in group),
                actual_exposed_target_count=len(exposed),T_exposed_hits=sum(x['before_hit'] for x in exposed),L_exposed_hits=sum(x['after_hit'] for x in exposed)))
