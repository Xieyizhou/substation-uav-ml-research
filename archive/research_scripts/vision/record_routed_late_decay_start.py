"""Record start only after actual optimizer steps, not just a launch request."""
from datetime import datetime,timezone
from pathlib import Path
import yaml
from scripts.vision.routed_late_decay_control import OUT,KEYS,SOURCE,SOURCE_KEYS,checked,freeze
...
def run():
    ...
    for k in KEYS[:2]:
        ...
        if v['status']!='real_late_decay_training_started' or v['learning_rates']!=p['learning_rates'][:10]:raise ValueError('No optimizer evidence')
        ...
        if cfg['model']!=p['initialization']['path'] or cfg.get('freeze') not in (None,0):raise ValueError('Initialization/freeze drift')
