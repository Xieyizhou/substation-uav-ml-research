"""Build hash-bound contact sheets for the 12 remaining negative frames."""
import json
import sys
...
def main():
    progress_path = BASE / "remaining-v1/capture-progress.json"
    progress = json.loads(progress_path.read_text())
    if progress["status"] != "complete_pending_review" or sum(row["captured"] for row in progress["runs"]) != 12:
        raise ValueError("Remaining negative capture is incomplete")
    out = BASE / "remaining-v1/review-v1"
    ...
    for run in progress["runs"]:
        ...
        for view in receipt["views"]:
            if view["status"] != "captured" or view["truth"]["objects"] or view["target_checks"]["observed_instance_labels"]:
                raise ValueError(f"Frame is not valid empty truth: {view['view_id']}")
            row = planned[view["view_id"]]
            image_path = Path(view["rgb_path"])
            image = Image.open(image_path).convert("RGB").resize((768, 432))
            tile = Image.new("RGB", (768, 468), (255, 255, 255)); tile.paste(image, (0, 36))
            ImageDraw.Draw(tile).text((8, 10), f"{run['run_id']} | {row['subject_family']} | {view['view_id'][:12]}", fill=(0, 0, 0))
            tiles.append(tile)
            frames.append({"run_id": run["run_id"], "view_id": view["view_id"], "pair_id": row["pair_id"],
                           "derivation_group": row["derivation_group"], "map_id": view["map_id"],
                           "subject_family": row["subject_family"], "subject_instance": row["object_id"],
                           "lighting_id": row["lighting_id"], "image_path": str(image_path), "image_sha256": file_sha256(image_path),
                           "truth_sha256": object_sha256(view["truth"]), "truth_object_count": 0, "skew_ms": view["skew_ms"],
                           "decision": "pending", "reason": None, "review_nature": "AI-assisted",
    ...
    for start in range(0, len(tiles), 4):
        ...
        path = out / f"review-sheet-{start // 4 + 1:02}.png"; sheet.save(path)
    ...
    result["identity"] = object_sha256(result); write_json(out / "manifest.json", result)
