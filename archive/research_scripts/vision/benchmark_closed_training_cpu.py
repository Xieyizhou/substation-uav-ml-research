...
from scripts.vision.closed_budget_engine import execute
from scripts.vision.train_closed_source_control import cleanup
OUT=BUDGET/'cpu-training-benchmark-v1'
...
def protocol():
    ...
    deps=[BUDGET/'design.json',BUDGET/'loader-completion.json',Path(__file__).resolve(),Path(__file__).with_name('closed_budget_engine.py')]
...
def worker(config,index):
    ...
    if n>3:raise ValueError('Probe attempt cap')
    ...
    for config,(threads,parallel) in CONFIGS.items():
        ...
        try:
            for i in range(2):
                ...
                proc=subprocess.Popen([sys.executable,'-u','-m','scripts.vision.benchmark_closed_training_cpu','--worker',config,str(i)],cwd=prior.ROOT,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
                procs.append(proc)
                if parallel==1:
