"""Run one frozen A/B/C/D development training cell and verify actual exposure."""
import argparse
import json
import sys
...
from scripts.vision.prepare_visual_augmentation_training import OUT, prepare
...
def train(arm,seed):
    ...
    schedule=protocol["schedules"][arm][str(seed)]; expected=[[path_for_member[mid] for mid in epoch] for epoch in schedule]
    actual=[]
...
if __name__=="__main__":
    parser=argparse.ArgumentParser(); parser.add_argument("--arm",choices=list("ABCD"),required=True); parser.add_argument("--seed",type=int,choices=[7,17,27],required=True); args=parser.parse_args()
