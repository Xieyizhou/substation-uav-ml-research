"""Evaluate development weights on the fresh targeted regression holdout."""
import json
import statistics
import sys
...
DATA = BASE / "targeted-regression-v1"
    ...
    for map_id in ("medium", "complex"):
        ...
        for row in receipt["views"]:
            ...
            rows.append({
                ...
                "distance_bin": "near" if spec["distance"] < 10 else "mid" if spec["distance"] < 15 else "far",
            })
    if len(rows) != 24:
        raise ValueError(f"Expected 24 targeted regression rows, got {len(rows)}")
    return rows
...
def main():
    ...
    report = {
        "status": "targeted_regression_evaluation_complete",
        "rows": len(rows),
        "results": results,
        "summary": summary,
