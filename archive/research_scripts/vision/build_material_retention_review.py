...
from scripts.vision.evaluate_material_retention_coverage import OUT,KEYS,prior,validate_record,low_dose
...
def main():
    ...
    for key in KEYS:
        path=low_dose.OUT/'evaluation'/f'VM-{key.rsplit("-",1)[1]}.json'
        r=prior.read(path);validate_record(r,'VM-'+key.rsplit('-',1)[1]);records[f'L-{key}']=r;paths.append(path)
    from scripts.vision.train_gray_body_control import OUT as GRAY
    for seed in (7,17,27):
        path=GRAY/'evaluation'/f'G-{seed}.json';r=prior.read(path);validate_record(r,f'G-{seed}');records[f'G-{seed}']=r;paths.append(path)
    identities={}
    for r in identity['corrected_target_records']:
        ...
        identities[k]=r
    transitions=[];losses={}
    for left,right in (('G','T'),('T','A'),('L-T','T'),('L-A','A')):
        for seed in (7,17,27):
    ...
    _,p,_=__import__('scripts.vision.train_material_retention_coverage',fromlist=['contract']).contract(KEYS[0])
