...
from scripts.vision.preflight_material_late_rehearsal import OUT,KEYS,protocol
from scripts.vision import train_material_retention_coverage as reference
def contract(key):
    if key not in KEYS:raise ValueError('Unknown late-rehearsal cell')
        ...
        for key in KEYS:
            ...
            try:
                with (attempt/'log.txt').open('x') as log:
                    proc=subprocess.Popen([sys.executable,'-u','-m','scripts.vision.train_material_late_rehearsal','--train','--worker',key],cwd=prior.ROOT,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
