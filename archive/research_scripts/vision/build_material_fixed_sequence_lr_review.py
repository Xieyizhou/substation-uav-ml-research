...
from scripts.vision.evaluate_material_fixed_sequence_lr import OUT,KEYS,prior,validate_record,low_dose
...
def main():
    ...
    from scripts.vision.train_material_retention_coverage import OUT as REFERENCE
    for seed in (7,17,27):
        path=REFERENCE/'evaluation'/f'T-{seed}.json';r=prior.read(path);validate_record(r,f'T-{seed}');records[f'T-{seed}']=r;paths.append(path)
    identities={}
    ...
    _,p,_=__import__('scripts.vision.train_material_fixed_sequence_lr',fromlist=['contract']).contract(KEYS[0])
