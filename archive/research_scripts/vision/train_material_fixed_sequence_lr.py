...
from scripts.vision.material_fixed_sequence_lr import OUT, freeze as protocol, check_pair
from scripts.vision import train_material_retention_coverage as reference
KEYS=('Q-7','Q-17','Q-27')
def contract(key):
    if key not in KEYS:raise ValueError('Unknown fixed-sequence LR cell')
    p=protocol();seed=int(key.split('-')[-1]);old,_,expected=reference.contract('T-'+str(seed))
    reference.complete('T-'+str(seed));check_pair(p,seed)
    ...
    if any(r[k]!=expected[k] for k in expected):raise ValueError('Preflight differs from historical actual inputs')
...
def worker(key):
    ...
    try:
        ...
        check_curve(yaml.safe_load(args.read_text()),list(csv.DictReader(curve.open())),seed,.00025)
        ...
        for key in KEYS:
            ...
            try:
                with (attempt/'log.txt').open('x') as log:
                    proc=subprocess.Popen([sys.executable,'-u','-m','scripts.vision.train_material_fixed_sequence_lr','--train','--worker',key],cwd=prior.ROOT,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
