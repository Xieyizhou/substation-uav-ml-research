...
from scripts.vision import evaluate_physical_low_light as evaluation
...
def run():
    ...
    with (root/'run.lock').open('a') as lock:
        ...
        for i in range(0,len(keys),parallel):
            ...
            try:
                for key in keys[i:i+parallel]:
                    ...
                    proc=subprocess.Popen([sys.executable,'-u','-m','scripts.vision.evaluate_physical_low_light','--evaluate','--cell',key],cwd=prior.ROOT,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
        ...
        prior.frozen(dest,dict(status='six_physical_light_models_evaluated_error_review_pending',wall_seconds=time.monotonic()-start,
