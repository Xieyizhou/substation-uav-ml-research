...
from scripts.vision.review_il_lower_rate import DEST,prior
from scripts.vision.ilr_review_notes import parse
from scripts.vision import record_bn_review as recorder
def main():
    recorder.DEST=DEST;recorder.parse=parse;r=recorder.main()
    deps=[DEST/'review.json',DEST/'evidence.json',Path(__file__).resolve(),Path(__file__).with_name('ilr_review_notes.py')]
