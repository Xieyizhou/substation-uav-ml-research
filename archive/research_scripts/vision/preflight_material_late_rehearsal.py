"""Traverse three actual loaders; no optimizer, backprop or validation."""
import traceback
...
def protocol():
    f=feasibility();old=original()
    p={k:old[k] for k in ('pool_rows','names','initialization','evaluation','held_members')}
    p.update(schedules={},brightness_factors={},training_config={},listings={})
    for k,c in f['cells'].items():
        p['schedules'][k]=c['sequence'];p['brightness_factors'][k]=c['brightness_factors']
        p['training_config'][k]=c['training_config'];p['listings'][k]=old['listings'][c['reference']]
        if Counter(c['sequence'])!=Counter(old['schedules'][c['reference']]):raise ValueError('Member count drift')
        if set(c['sequence'])&set(p['held_members']):raise ValueError('Held member restored')
    paths=[OUT/'feasibility.json',SOURCE/'protocol.json',SOURCE/'decision-protocol.json',Path(__file__).resolve()]
    p.update(status='configuration_frozen_actual_loader_pending',training_started=False,
        acceptance_policy=prior.read(SOURCE/'decision-protocol.json'),
        inputs={str(x):prior.file_sha256(x) for x in paths})
    dest=OUT/'protocol.json'
    if dest.exists():
        r=prior.read(dest);prior.verify(r)
        if any(r[k]!=p[k] for k in ('schedules','brightness_factors','training_config','listings')):raise ValueError('Configuration changed')
        return r
