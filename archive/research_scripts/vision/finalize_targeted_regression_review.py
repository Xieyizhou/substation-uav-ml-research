"""Finalize manual semantic review for fresh targeted regression frames."""
import json
import sys
...
def main():
    ...
    for run in manifest["runs"]:
        ...
        for row in receipt["views"]:
            if row.get("status") != "captured" or row["view_id"] not in selected or row.get("expected_class_present") is not True:
                raise ValueError(f"Invalid targeted row: {row.get('view_id')}")
            ...
            frames.append({
                ...
                "review_note": "Fresh targeted frame retained as a development-only regression holdout.",
                "training_admitted": False,
            })
    if len(frames) != 24 or len({row["view_id"] for row in frames}) != 24:
        raise ValueError("Expected 24 unique targeted regression frames")
    result = {
        "status": "reviewed", "review_type": "manual_semantic_review_targeted_regression",
