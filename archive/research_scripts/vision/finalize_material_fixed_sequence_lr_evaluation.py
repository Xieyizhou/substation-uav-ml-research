...
from scripts.vision.evaluate_material_fixed_sequence_lr import OUT, KEYS, prior, complete, validate_record, verify_binding, baseline_verify
from scripts.vision.record_material_fixed_sequence_lr_review import validate


TESTS = ('test_material_fixed_sequence_lr_evaluation', 'test_material_fixed_sequence_lr_review',
         'test_material_late_fit', 'test_material_fixed_sequence_lr',
...
def main():
    ...
    if len(e['transitions'])!=720 or s['fp_predictions']!=7 or s['unique_loss_targets']!=41:
        raise ValueError('Incomplete error inventory')
    if summary['matching_conflicts']!=0:raise ValueError('Unresolved matching conflict')
    from scripts.vision.diagnose_material_fixed_sequence_lr_fit import OUT as FIT, freeze, validate as fit_validate
    fp=freeze()
    for key in KEYS:
    ...
    report=prior.ROOT/'docs/results/ml_material_fixed_sequence_lr_evaluation_20260912.md'
    ...
    return prior.frozen(dest,dict(status='development_evaluation_and_explicit_review_complete_not_selected',
        ...
        next_direction='audit_existing_controlled_material_exposure_feasibility_no_new_training',
