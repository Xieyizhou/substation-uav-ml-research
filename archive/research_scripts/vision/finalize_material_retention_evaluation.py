...
from scripts.vision.evaluate_material_retention_coverage import OUT, KEYS, prior, complete, validate_record, verify_binding, baseline_verify
from scripts.vision.record_material_retention_review import validate


TESTS = ('test_material_retention_evaluation', 'test_material_retention_review',
         'test_material_retention_entry', 'test_material_retention_coverage',
...
def main():
    ...
    if len(e['transitions'])!=2880 or s['fp_predictions']!=15 or s['unique_loss_targets']!=84:
        raise ValueError('Incomplete error inventory')
    if summary['matching_conflicts']!=0:raise ValueError('Unresolved matching conflict')
    from scripts.vision.diagnose_material_retention_transfer import OUT as FIT, freeze, validate as validate_fit
    p=freeze()
    for key in KEYS:
        f=FIT/(key+'.json'); validate_fit(prior.read(f),key,p); paths.append(f)
    for f in (FIT/'protocol.json',FIT/'summary.json'):
        prior.verify(prior.read(f)); paths.append(f)
    ...
    report=prior.ROOT/'docs/results/ml_material_retention_evaluation_20260912.md'
    ...
    return prior.frozen(dest,dict(status='development_evaluation_and_explicit_review_complete_not_selected',
        numerical_passed={k:v['passed'] for k,v in summary['policy_results'].items()},selected_candidate=None,
        ...
        next_direction='design_fixed_member_single_variable_exposure_order_fit_control',
