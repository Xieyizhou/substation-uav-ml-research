...
from scripts.vision.train_compensated_double_dose import OUT, KEYS, complete, contract, prior
...
def finish():
    ...
    low_records={}
    for key in KEYS:
        low_dose.complete(key);ep=low_dose.OUT/'evaluation'/f'{key}.json'
        rec=prior.read(ep);validate_record(rec,key)
        c=prior.read(low_dose.OUT/'training'/key/'completion.json')
        if rec['inputs'].get(c['weights'])!=c['weights_sha256']:raise ValueError('Low-dose reference weight mismatch')
        low_records[key]=rec;paths.append(ep)
    low_groups={a:aggregate([low_records[f'{a}-{s}'] for s in (7,17,27)]) for a in ('V','VM')}
    ...
    paths += refs + [hp,OUT/'protocol.json',OUT/'decision-protocol.json',Path(__file__).resolve()]
    decision=prior.read(OUT/'decision-protocol.json');prior.verify(decision)
    if decision['acceptance_policy']!=policy['acceptance_policy'] or decision['retention']!=policy['retention']:raise ValueError('Frozen gate drift')
    return prior.frozen(OUT/'evaluation/summary.json',dict(status='numerical_complete_explicit_error_review_pending',aggregate=groups,
        dose_label='108 new-member exposures, compared with frozen54 counterparts',
        low_dose_aggregate=low_groups,
        dose_retention={a:direct_checks(groups[a],low_groups[a]) for a in groups},
        dose_paired_changes={a:{str(s):paired_change(low_records[f'{a}-{s}']['rows'],records[f'{a}-{s}']['rows']) for s in (7,17,27)} for a in groups},
