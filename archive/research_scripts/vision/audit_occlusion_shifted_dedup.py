"""Audit exact and dHash duplication for occlusion-shifted frames."""
import json
import sys
...
def main():
    ...
    if review.get("accepted") != 24 or review.get("held"):
        raise ValueError("Occlusion-shift review is not closed")
    prior = {}
    for receipt_path in ROOT.joinpath("data/research").rglob("collection-receipt.json"):
        if "protected" in str(receipt_path) or "occlusion-shift-v1" in str(receipt_path): continue
    ...
    for row in sorted(review["frames"], key=lambda item: item["view_id"]):
        ...
        status = ("exact_duplicate_prior" if exact_prior else
                  "exact_duplicate_occlusion_shift" if exact_internal else
                  "near_similarity_review" if nearest[0] <= 2 else "retained")
        decisions.append({**row, "perceptual_hash": f"{value:016x}", "nearest_prior_hamming_distance": nearest[0],
                          "nearest_prior_view_id": nearest[1], "exact_prior_duplicate": exact_prior,
                          "exact_occlusion_shift_duplicate": exact_internal, "status": status, "training_admitted": False})
        seen.add(row["image_sha256"])
    result = {
        "status": "occlusion_shifted_dedup_audit_complete", "frames": len(decisions),
        ...
        "limits": ["Non-protected historical receipt scope only.", "dHash distance <=2 is a quarantine trigger, not duplicate proof."],
    }
