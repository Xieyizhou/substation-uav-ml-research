...
class SandboxOperatorTests(unittest.TestCase):
    ...
    def test_http_mutations_require_server_token(self):
        ...
        try:
            ...
            setup = connection.getresponse()
            setup_payload = json.loads(setup.read())
            self.assertEqual(setup.status, 200)
            self.assertEqual(setup_payload["setup_schema_version"], 2)
            self.assertEqual(setup_payload["profile"], "development")
            self.assertEqual(len(setup_payload["journey"]), 4)
            self.assertIn("workbench", setup_payload["artifact_locations"])
