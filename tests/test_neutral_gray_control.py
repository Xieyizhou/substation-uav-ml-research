...
from scripts.vision import neutral_gray_control as control
...
class ControlledSwap(unittest.TestCase):
    def setUp(self):
        ...
        common = dict(image_path=str(image), image_sha256=control.prior.file_sha256(image),
                      ...
                      subset='positive', class_instances={'transformer': 1}, lineage_id='pose', variant='neutral')
        old = [dict(common, member_id=f'low-{i}', source_member_id=f'src-{i}', illumination='physical_low_neutral') for i in range(16)]
        self.new = [dict(common, member_id=f'cool-{i}', source_member_id=f'low-{i}', material_setting='neutral_0.35') for i in range(16)]
        ...
        for seed in (7,17,27):
            key = f'IC1000-{seed}'
            self.ref['schedules'][key] = [f'low-{i%16}' for i in range(300)]+['negative']*5700
    ...
    def test_negative_position(self):
        self.p['schedules']['ICG1000-7'][301] = 'cool-1'
        with self.assertRaises(ValueError): control.constraints(self.p,self.ref,self.new)
    def test_brightness(self):
        self.p['brightness_factors']['ICG1000-7'][0] = .9
        with self.assertRaises(ValueError): control.constraints(self.p,self.ref,self.new)
    def test_learning_rate(self):
        self.p['training_config']['ICG1000-7']['lr0'] = .00025
        with self.assertRaises(ValueError): control.constraints(self.p,self.ref,self.new)
    ...
    def test_ledger(self):
        self.p['ledger']['ICG1000-7'][0]['classes']['transformer'] += 1
        with self.assertRaises(ValueError): control.constraints(self.p,self.ref,self.new)
...
class ExplicitReview(unittest.TestCase):
    def setUp(self):
        ...
        self.e = dict(identity='e',image_sha256='i',events=[dict(event_id='L01:a',crop_sha256='c',object_id='a')])
        self.r = dict(full_frames_viewed=['L01'], decisions=[dict(event_id='L01:a', evidence_identity='e',image_sha256='i',object_id='a',review_time='2026-09-14T00:00:00Z',crop_sha256='c',status='content_sufficient_for_bounded_research', reason='body visible',review_nature='AI辅助审核', training_admitted=False,promotable=False)])
    def validate(self):
        with patch('scripts.vision.neutral_gray_dataset.prior.verify'), patch('scripts.vision.neutral_gray_dataset.evidence',return_value=self.e):
    ...
    def test_image_drift(self):
        self.r['decisions'][0]['image_sha256']='changed'
        with self.assertRaises(ValueError): self.validate()
    def test_instance_drift(self):
        self.r['decisions'][0]['object_id']='wrong'
        with self.assertRaises(ValueError): self.validate()


class EntryGate(unittest.TestCase):
    def setUp(self):
        from scripts.vision import train_neutral_gray as train
        self.train=train
