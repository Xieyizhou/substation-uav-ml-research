...
from scripts.vision.closed_source_error_observations import OBS
...
class ExplicitReviewTests(unittest.TestCase):
    ...
    def test_valid(self):
        e,d=self.fixture()
        with patch('scripts.vision.record_closed_source_error_review.prior.file_sha256',side_effect=lambda p: {'impath':'im','pagepath':'page'}[p]):
            validate(e,[d])
    ...
    def test_missing_duplicate_stale_prediction_source_truth(self):
        ...
        for change in ('missing','duplicate','image','page','prediction','source','truth','certification','unknown_pass'):
            ...
            with self.subTest(change=change), patch('scripts.vision.record_closed_source_error_review.prior.file_sha256',side_effect=lambda p:{'impath':'im','pagepath':'page'}[p]):
                with self.assertRaises(ValueError):validate(e,ds)

    def test_complete_explicit_observation_inventory(self):
        self.assertEqual(set(OBS),{f'E{i:02}' for i in range(1,45)})
        self.assertEqual(sum(map(len,OBS.values())),49)
        self.assertEqual(sum(c=='unknown_instance_content' for v in OBS.values() for c,_ in v),12)

    def test_frozen_evidence_and_isolated_render(self):
        from scripts.vision.confirm_closed_source_error_review_v2 import DEST, prior, isolated_render
        ...
        with TemporaryDirectory() as tmp:
            out=isolated_render((e['events'][0],tmp))
            self.assertTrue(out['page_path'].startswith(tmp))
        prior.verify(e)
        d=prior.read(DEST/'diagnostic-confirmation-v2.json');prior.verify(d)
        self.assertEqual(len(d['matching_competitions']),3)
        self.assertEqual(len(e['transitions']),720)
