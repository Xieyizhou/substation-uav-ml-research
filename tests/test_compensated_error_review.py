...
from scripts.vision.record_compensated_error_review import validate
from scripts.vision.compensated_review_observations import OBS
...
class ExplicitReviewTests(unittest.TestCase):
    ...
    def test_valid(self):
        e,d=self.fixture()
        with patch('scripts.vision.record_compensated_error_review.prior.file_sha256',side_effect=lambda p: {'impath':'im','pagepath':'page'}[p]):
            validate(e,[d])
    ...
    def test_missing_duplicate_stale_prediction_source_truth(self):
        ...
        for change in ('missing','duplicate','image','page','prediction','source','truth','certification','unknown_pass'):
            ...
            with self.subTest(change=change), patch('scripts.vision.record_compensated_error_review.prior.file_sha256',side_effect=lambda p:{'impath':'im','pagepath':'page'}[p]):
                with self.assertRaises(ValueError):validate(e,ds)

    def test_complete_explicit_observation_inventory(self):
        self.assertEqual(set(OBS),{f'E{i:02}' for i in range(1,79)})
        self.assertEqual(sum(map(len,OBS.values())),84)
        self.assertEqual(sum(c=='unknown_instance_content' for v in OBS.values() for c,_ in v),18)
